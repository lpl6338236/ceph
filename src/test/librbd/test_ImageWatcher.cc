...
TEST_F(TestImageWatcher, TryLockWithUserSharedLocked) {
  ...
  RWLock::WLocker l(ictx->leader_lock);
  ASSERT_EQ(-EINVAL, ictx->image_watcher->try_lock());
  ASSERT_FALSE(ictx->image_watcher->is_lock_owner());
...
TEST_F(TestImageWatcher, RequestLock) {
  ...
  {
    ENCODE_START(1, 1, bl);
    ::encode(static_cast<uint8_t>(NOTIFY_OP_RELEASED_LOCK), bl);
    ENCODE_FINISH(bl);
  }
...
TEST_F(TestImageWatcher, RequestLockTryLockRace) {
  ...
  librbd::ImageCtx *ictx;
  ASSERT_EQ(0, open_image(m_image_name, &ictx));
  ASSERT_EQ(0, lock_image(*ictx, LOCK_EXCLUSIVE, "manual"));
...
TEST_F(TestImageWatcher, RequestLockPostTryLockFailed) {
  ...
  librbd::ImageCtx *ictx;
  ASSERT_EQ(0, open_image(m_image_name, &ictx));
  ASSERT_EQ(0, lock_image(*ictx, LOCK_EXCLUSIVE, "manually 1234"));
