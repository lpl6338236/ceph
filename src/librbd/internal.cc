...
bool *old_format, uint64_t *size) {
	...
	ldout(cct, 20)
				<< "detect format of " << name << " : "
						<< (old_format ?
								(*old_format ? "old" : "new") : "don't care")
						<< dendl;
	return 0;
}
...
void init_rbd_header(struct rbd_obj_header_ondisk& ondisk, uint64_t size,
	...
	snprintf(ondisk.block_name, sizeof(ondisk.block_name), "rb.%x.%x.%x", hi,
			lo, extra);
...
void trim_image(ImageCtx *ictx, uint64_t newsize, ProgressContext& prog_ctx) {
	...
	ldout(cct, 10)
				<< "trim_image " << size << " -> " << newsize << " periods "
						<< num_period << " discard to offset " << delete_off
						<< " delete objects " << delete_start << " to "
						<< (num_objects - 1) << dendl;

	SimpleThrottle throttle(cct->_conf->rbd_concurrent_management_ops, true);
	if (delete_start < num_objects) {
		ldout(cct, 2)
					<< "trim_image objects " << delete_start << " to "
							<< (num_objects - 1) << dendl;
		for (uint64_t i = delete_start; i < num_objects; ++i) {
			...
			librados::AioCompletion *rados_completion =
					librados::Rados::aio_create_completion(req_comp, NULL,
							rados_ctx_cb);
...
int read_header_bl(IoCtx& io_ctx, const string& header_oid, bufferlist& header,
		uint64_t *ver) {
	int r;
	uint64_t off = 0;
...
int notify_change(IoCtx& io_ctx, const string& oid, uint64_t *pver,
	...
	if (ictx) {
		...
		ldout(ictx->cct, 20)
					<< "notify_change refresh_seq = " << ictx->refresh_seq
...
int rollback_image(ImageCtx *ictx, uint64_t snap_id,
	...
	uint64_t numseg = Striper::get_num_objects(ictx->layout,
			ictx->get_current_size());
	...
	for (uint64_t i = 0; i < numseg; i++) {
		...
		ldout(cct, 10)
					<< "scheduling selfmanaged_snap_rollback on " << oid
							<< " to " << snap_id << dendl;
		rados_completion->release();
	...
	if (r < 0) {
		ldout(cct, 10)
					<< "failed to rollback at least one object: "
							<< cpp_strerror(r) << dendl;
		return r;
	}
	return 0;
...
int list(IoCtx& io_ctx, vector<string>& names) {
	...
	if (bl.length()) {
		...
		::decode(header, p);
		::decode(m, p);
		for (map<string, bufferlist>::iterator q = m.begin(); q != m.end();
				++q) {
	...
	do {
		...
		cls_client::dir_list(&io_ctx, RBD_DIRECTORY, last_read, max_read,
				&images);
...
int snap_create(ImageCtx *ictx, const char *snap_name) {
	ldout(ictx->cct, 20)
				<< "snap_create " << ictx << " " << snap_name << dendl;
...
static int scan_for_parents(ImageCtx *ictx, parent_spec &pspec,
	...
	if (pspec.pool_id != -1) {
		...
		for (it = ictx->snap_info.begin(); it != ictx->snap_info.end(); ++it) {
...
int snap_remove(ImageCtx *ictx, const char *snap_name) {
	ldout(ictx->cct, 20)
				<< "snap_remove " << ictx << " " << snap_name << dendl;
...

	{
		...
		if (r < 0) {
			lderr(ictx->cct)
						<< "snap_remove: can't get parent spec" << dendl;
			return r;
		}

		if (ictx->parent_md.spec != our_pspec
				&& (scan_for_parents(ictx, our_pspec, snap_id) == -ENOENT)) {
			r = cls_client::remove_child(&ictx->md_ctx, RBD_CHILDREN, our_pspec,
					ictx->id);
			if (r < 0)
				return r;
		}
...
int snap_protect(ImageCtx *ictx, const char *snap_name) {
	ldout(ictx->cct, 20)
				<< "snap_protect " << ictx << " " << snap_name << dendl;
	...
	if ((features & RBD_FEATURE_LAYERING) == 0) {
		lderr(ictx->cct)
					<< "snap_protect: image must support layering" << dendl;
		return -ENOSYS;
	}
	...
	r = cls_client::set_protection_status(&ictx->md_ctx, ictx->header_oid,
			snap_id, RBD_PROTECTION_STATUS_PROTECTED);
	if (r < 0)
		return r;
	notify_change(ictx->md_ctx, ictx->header_oid, NULL, ictx);
	return 0;
...
int snap_unprotect(ImageCtx *ictx, const char *snap_name) {
	ldout(ictx->cct, 20)
				<< "snap_unprotect " << ictx << " " << snap_name << dendl;
	...
	if ((features & RBD_FEATURE_LAYERING) == 0) {
		lderr(ictx->cct)
					<< "snap_unprotect: image must support layering" << dendl;
		return -ENOSYS;
	}
	snap_t snap_id = ictx->get_snap_id(snap_name);
	if (snap_id == CEPH_NOSNAP)
	...
	if (is_unprotected) {
		lderr(ictx->cct)
					<< "snap_unprotect: snapshot is already unprotected"
							<< dendl;
		return -EINVAL;
	}

	r = cls_client::set_protection_status(&ictx->md_ctx, ictx->header_oid,
			snap_id, RBD_PROTECTION_STATUS_UNPROTECTING);
	if (r < 0)
		return r;
	notify_change(ictx->md_ctx, ictx->header_oid, NULL, ictx);
	...
	for (std::list<std::string>::const_iterator it = pools.begin();
			it != pools.end(); ++it) {
		int64_t pool_id = rados.pool_lookup(it->c_str());
		int64_t base_tier;
		...
		if (r < 0) {
			lderr(ictx->cct)
						<< "snap_unprotect: can't create ioctx for pool " << *it
								<< dendl;
			goto reprotect_and_return_err;
		}
		r = cls_client::get_children(&pool_ioctx, RBD_CHILDREN, pspec,
				children);
		// key should not exist for this parent if there is no entry
		if (((r < 0) && (r != -ENOENT))) {
			lderr(ictx->cct)
						<< "can't get children for pool " << *it << dendl;
			goto reprotect_and_return_err;
		}
		// if we found a child, can't unprotect
		if (r == 0) {
			lderr(ictx->cct)
						<< "snap_unprotect: can't unprotect; at least "
	...
	r = cls_client::set_protection_status(&ictx->md_ctx, ictx->header_oid,
			snap_id, RBD_PROTECTION_STATUS_UNPROTECTED);
	if (r < 0) {
		lderr(ictx->cct)
					<< "snap_unprotect: error setting unprotected status"
							<< dendl;
		goto reprotect_and_return_err;
	}
	notify_change(ictx->md_ctx, ictx->header_oid, NULL, ictx);
	...
	reprotect_and_return_err: int proterr = cls_client::set_protection_status(
			&ictx->md_ctx, ictx->header_oid, snap_id,
			RBD_PROTECTION_STATUS_PROTECTED);
	if (proterr < 0) {
		lderr(ictx->cct)
					<< "snap_unprotect: can't reprotect image" << dendl;
	}
	notify_change(ictx->md_ctx, ictx->header_oid, NULL, ictx);
	return r;
}
...
bool *is_protected) {
	ldout(ictx->cct, 20)
				<< "snap_is_protected " << ictx << " " << snap_name << dendl;
...
int create_v1(IoCtx& io_ctx, const char *imgname, uint64_t bid, uint64_t size,
		int order) {
	CephContext *cct = (CephContext *) io_ctx.cct();
	ldout(cct, 2)
...
int create_v2(IoCtx& io_ctx, const char *imgname, uint64_t bid, uint64_t size,
	...
	if (r < 0) {
		lderr(cct)
					<< "error setting image id: " << cpp_strerror(r) << dendl;
		goto err_remove_id;
	}
	...
	if (r < 0) {
		lderr(cct)
					<< "error writing header: " << cpp_strerror(r) << dendl;
		goto err_remove_from_dir;
	}

	if ((stripe_unit || stripe_count)
			&& (stripe_count != 1 || stripe_unit != (1ull << order))) {
		r = cls_client::set_stripe_unit_count(&io_ctx, header_oid, stripe_unit,
				stripe_count);
		if (r < 0) {
			lderr(cct)
						<< "error setting striping parameters: "
								<< cpp_strerror(r) << dendl;
	...
	err_remove_header: remove_r = io_ctx.remove(header_oid);
	if (remove_r < 0) {
		lderr(cct)
					<< "error cleaning up image header after creation failed: "
							<< dendl;
	}
	err_remove_from_dir: remove_r = cls_client::dir_remove_image(&io_ctx,
			RBD_DIRECTORY, imgname, id);
	if (remove_r < 0) {
		lderr(cct)
	...
	err_remove_id: remove_r = io_ctx.remove(id_obj);
	if (remove_r < 0) {
		lderr(cct)
...
bool old_format, uint64_t features, int *order, uint64_t stripe_unit,
		uint64_t stripe_count) {
	if (!order)
		return -EINVAL;
	...
	CephContext *cct = (CephContext *) io_ctx.cct();
	ldout(cct, 20)
				<< "create " << &io_ctx << " name = " << imgname << " size = "
						<< size << " old_format = " << old_format
						<< " features = " << features << " order = " << *order
						<< " stripe_unit = " << stripe_unit
						<< " stripe_count = " << stripe_count << dendl;
	...
	if (r != -ENOENT) {
		if (r) {
			lderr(cct)
						<< "Could not tell if " << imgname << " already exists"
								<< dendl;
			return r;
		}
		lderr(cct)
					<< "rbd image " << imgname << " already exists" << dendl;
		return -EEXIST;
	}
	...
	if (!old_format && (features & RBD_FEATURE_STRIPINGV2) && !stripe_unit
			&& !stripe_count) {
	...
	if ((stripe_unit || stripe_count)
		...
		lderr(cct)
							<< "STRIPINGV2 and format 2 or later required for non-default striping"
							<< dendl;
		return -EINVAL;
	}
...
int clone(IoCtx& p_ioctx, const char *p_name, const char *p_snap_name,
		IoCtx& c_ioctx, const char *c_name, uint64_t features, int *c_order,
		uint64_t stripe_unit, int stripe_count) {
	CephContext *cct = (CephContext *) p_ioctx.cct();
	ldout(cct, 20)
				<< "clone " << &p_ioctx << " name " << p_name << " snap "
						<< p_snap_name << "to child " << &c_ioctx << " name "
						<< c_name << " features = " << features << " order = "
						<< *c_order << " stripe_unit = " << stripe_unit
						<< " stripe_count = " << stripe_count << dendl;
	...
	if (r != -ENOENT) {
		lderr(cct)
					<< "rbd image " << c_name << " already exists" << dendl;
		return -EEXIST;
	}
	...
	if (r < 0) {
		lderr(cct)
					<< "error opening parent image: " << cpp_strerror(-r)
							<< dendl;
		return r;
	}

	parent_spec pspec(p_ioctx.get_id(), p_imctx->id, p_imctx->snap_id);
	...
	r = create(c_ioctx, c_name, size, false, features, &order, stripe_unit,
			stripe_count);
	if (r < 0) {
		lderr(cct)
					<< "error creating child: " << cpp_strerror(r) << dendl;
		goto err_close_parent;
	}
	...
	if (r < 0) {
		lderr(cct)
					<< "Error opening new image: " << cpp_strerror(r) << dendl;
		goto err_remove;
	}
	...
	err_remove_child: remove_r = cls_client::remove_child(&c_ioctx,
			RBD_CHILDREN, pspec, c_imctx->id);
	if (remove_r < 0) {
		lderr(cct)
					<< "Error removing failed clone from list of children: "
							<< cpp_strerror(remove_r) << dendl;
	}
	err_close_child: close_image(c_imctx);
	err_remove: remove_r = remove(c_ioctx, c_name, no_op);
	if (remove_r < 0) {
		lderr(cct)
					<< "Error removing failed clone: " << cpp_strerror(remove_r)
							<< dendl;
	}
	err_close_parent: close_image(p_imctx);
	return r;
}
...
int rename(IoCtx& io_ctx, const char *srcname, const char *dstname) {
	...
	if (r < 0) {
		lderr(cct)
					<< "error finding source object: " << cpp_strerror(r)
							<< dendl;
		return r;
	}
	...
	if (!old_format) {
		...
		if (r < 0) {
			lderr(cct)
						<< "error reading image id: " << cpp_strerror(r)
								<< dendl;
			return r;
		}
	}
	...
	if (r < 0) {
		lderr(cct)
					<< "error reading source object: " << src_oid << ": "
							<< cpp_strerror(r) << dendl;
		return r;
	}
	...
	do {
		...
		if (r < 0) {
			lderr(cct)
						<< "error reading source object omap values: "
								<< cpp_strerror(r) << dendl;
			return r;
		}
	...
	if (r < 0 && r != -ENOENT) {
		lderr(cct)
					<< "error checking for existing image called " << dstname
							<< ":" << cpp_strerror(r) << dendl;
		return r;
	}
	if (r == 0) {
		lderr(cct)
					<< "rbd image " << dstname << " already exists" << dendl;
		return -EEXIST;
	}
	...
	if (r < 0) {
		lderr(cct)
					<< "error writing destination object: " << dst_oid << ": "
							<< cpp_strerror(r) << dendl;
		return r;
	}
	...
	if (old_format) {
		...
		if (r < 0) {
			...
			lderr(cct)
						<< "couldn't add " << dstname << " to directory: "
								<< cpp_strerror(r) << dendl;
			return r;
		}
		...
		if (r < 0) {
			lderr(cct)
						<< "warning: couldn't remove old entry from directory ("
								<< srcname << ")" << dendl;
		}
	} else {
		r = cls_client::dir_rename_image(&io_ctx, RBD_DIRECTORY, srcname,
				dstname, id);
		if (r < 0) {
			lderr(cct)
						<< "error updating directory: " << cpp_strerror(r)
								<< dendl;
			return r;
		}
	}
	...
	if (r < 0 && r != -ENOENT) {
		lderr(cct)
					<< "warning: couldn't remove old source object (" << src_oid
							<< ")" << dendl;
	}
...
int open_parent(ImageCtx *ictx) {
	...
	if (r < 0) {
		lderr(ictx->cct)
					<< "error looking up name for pool id " << pool_id << ": "
							<< cpp_strerror(r) << dendl;
		return r;
	}
	...
	if (r < 0) {
		lderr(ictx->cct)
					<< "parent snapshot does not exist" << dendl;
	...
	if (r < 0) {
		lderr(ictx->cct)
					<< "error refreshing parent snapshot " << ictx->parent->id
...
int get_parent_info(ImageCtx *ictx, string *parent_pool_name,
	...
	if (ictx->snap_id == CEPH_NOSNAP) {
		...
		if (r < 0) {
			lderr(ictx->cct)
						<< "Can't find snapshot id" << ictx->snap_id << dendl;
			return r;
		}
	...
	if (parent_pool_name) {
		Rados rados(ictx->md_ctx);
		r = rados.pool_reverse_lookup(parent_spec.pool_id, parent_pool_name);
		if (r < 0) {
			lderr(ictx->cct)
	...
	if (parent_snap_name) {
		...
		r = ictx->parent->get_snap_name(parent_spec.snap_id, parent_snap_name);
		if (r < 0) {
			lderr(ictx->cct)
	...
	if (parent_name) {
		...
		if (r < 0) {
			lderr(ictx->cct)
						<< "error getting parent image name: "
								<< cpp_strerror(r) << dendl;
			return r;
		}
	}
...
int remove(IoCtx& io_ctx, const char *imgname, ProgressContext& prog_ctx) {
	...
	if (r < 0) {
		ldout(cct, 2)
					<< "error opening image: " << cpp_strerror(-r) << dendl;
	} else {
	...
	if (old_format || unknown_format) {
		ldout(cct, 2)
					<< "removing rbd image from directory..." << dendl;
		r = tmap_rm(io_ctx, imgname);
		old_format = (r == 0);
		if (r < 0 && !unknown_format) {
			lderr(cct)
						<< "error removing img from old-style directory: "
								<< cpp_strerror(-r) << dendl;
			return r;
		}
	}
	if (!old_format) {
		...
		if (r < 0 && r != -ENOENT) {
			lderr(cct)
						<< "error getting id of image" << dendl;
			return r;
		}
		...
		if (r < 0) {
			lderr(cct)
						<< "error removing img from new-style directory: "
								<< cpp_strerror(-r) << dendl;
			return r;
		}
	}
...
int resize_helper(ImageCtx *ictx, uint64_t size, ProgressContext& prog_ctx) {
	...
	if (size == ictx->size) {
		ldout(cct, 2)
					<< "no change in size (" << ictx->size << " -> " << size
							<< ")" << dendl;
		return 0;
	}
	...
	if (size > ictx->size) {
		ldout(cct, 2)
					<< "expanding image " << ictx->size << " -> " << size
							<< dendl;
					// TODO: make ictx->set_size
	} else {
		ldout(cct, 2)
					<< "shrinking image " << ictx->size << " -> " << size
							<< dendl;
	...
	if (r == -ERANGE)
		lderr(cct)
					<< "operation might have conflicted with another client!"
							<< dendl;
	if (r < 0) {
		lderr(cct)
					<< "error writing header: " << cpp_strerror(-r) << dendl;
...
bool snap_exists(ImageCtx *ictx, const char *snap_name) {
	ldout(ictx->cct, 20)
				<< "snap_exists " << ictx << " " << snap_name << dendl;
...
int add_snap(ImageCtx *ictx, const char *snap_name) {
	...
	if (r < 0) {
		lderr(ictx->cct)
					<< "failed to create snap id: " << cpp_strerror(-r) << dendl;
		return r;
	}
	...
	if (ictx->old_format) {
		...
		r = cls_client::snapshot_add(&ictx->md_ctx, ictx->header_oid, snap_id,
				snap_name);
	}

	if (r < 0) {
		lderr(ictx->cct)
					<< "adding snapshot to header failed: " << cpp_strerror(r)
							<< dendl;
		return r;
	}
...
int rm_snap(ImageCtx *ictx, const char *snap_name) {
	int r;
	if (ictx->old_format) {
		r = cls_client::old_snapshot_remove(&ictx->md_ctx, ictx->header_oid,
				snap_name);
	} else {
		RWLock::RLocker l(ictx->snap_lock);
		r = cls_client::snapshot_remove(&ictx->md_ctx, ictx->header_oid,
				ictx->get_snap_id(snap_name));
	}

	if (r < 0) {
		lderr(ictx->cct)
					<< "removing snapshot from header failed: "
							<< cpp_strerror(r) << dendl;
		return r;
	}
...
int ictx_check(ImageCtx *ictx) {
	...
	if (needs_refresh) {
		...
		if (r < 0) {
			lderr(cct)
						<< "Error re-reading rbd header: " << cpp_strerror(-r)
								<< dendl;
			return r;
		}
	}
...
int refresh_parent(ImageCtx *ictx) {
	...
	if (ictx->parent) {
		...
		if (!overlap
				...
				|| ictx->parent->snap_id
						!= ictx->get_parent_snap_id(ictx->snap_id)) {
	...
	if (ictx->get_parent_pool_id(ictx->snap_id) > -1 && !ictx->parent) {
		...
		if (r < 0) {
			lderr(ictx->cct)
						<< "error opening parent snapshot: " << cpp_strerror(r)
								<< dendl;
			return r;
		}
	}
	...
	vector<uint8_t> snap_protection;
	{
		RWLock::WLocker l(ictx->snap_lock);
		{
			...
			if (ictx->old_format) {
				...
				r = cls_client::old_snapshot_list(&ictx->md_ctx,
						ictx->header_oid, &snap_names, &snap_sizes, &new_snapc);
				if (r < 0) {
					lderr(cct)
				...
				r = rados::cls::lock::get_lock_info(&ictx->md_ctx,
						ictx->header_oid,
						RBD_LOCK_NAME, &ictx->lockers, &lock_type,
						&ictx->lock_tag);
				...
				do {
					...
					r = cls_client::get_mutable_metadata(&ictx->md_ctx,
							ictx->header_oid, &ictx->size, &ictx->features,
							&incompatible_features, &ictx->lockers,
							&ictx->exclusive_locked, &ictx->lock_tag,
							&new_snapc, &ictx->parent_md);
					if (r < 0) {
						lderr(cct)
									<< "Error reading mutable metadata: "
											<< cpp_strerror(r) << dendl;
						return r;
					}

					uint64_t unsupported = incompatible_features
							& ~RBD_FEATURES_ALL;
					if (unsupported) {
						lderr(ictx->cct)
									<< "Image uses unsupported features: "
											<< unsupported << dendl;
						return -ENOSYS;
					}

					r = cls_client::snapshot_list(&(ictx->md_ctx),
							ictx->header_oid, new_snapc.snaps, &snap_names,
							&snap_sizes, &snap_features, &snap_parents,
							&snap_protection);
					// -ENOENT here means we raced with snapshot deletion
					if (r < 0 && r != -ENOENT) {
						lderr(ictx->cct)
									<< "snapc = " << new_snapc << dendl;
						lderr(ictx->cct)
									<< "Error listing snapshots: "
											<< cpp_strerror(r) << dendl;
						return r;
			...
			for (size_t i = 0; i < new_snapc.snaps.size(); ++i) {
				...
				vector<snap_t>::const_iterator it = find(ictx->snaps.begin(),
						ictx->snaps.end(), new_snapc.snaps[i].val);
				if (it == ictx->snaps.end()) {
					new_snap = true;
					ldout(cct, 20)
								<< "new snapshot id=" << new_snapc.snaps[i].val
										<< " name=" << snap_names[i] << " size="
...
int snap_rollback(ImageCtx *ictx, const char *snap_name,
	...
	CephContext *cct = ictx->cct;
	ldout(cct, 20)
				<< "snap_rollback " << ictx << " snap = " << snap_name << dendl;
	...
	ldout(cct, 2)
				<< "resizing to snapshot size..." << dendl;
	NoOpProgressContext no_op;
	r = resize_helper(ictx, new_size, no_op);
	if (r < 0) {
		lderr(cct)
					<< "Error resizing to snapshot size: " << cpp_strerror(-r)
							<< dendl;
		return r;
	}
	...
	if (r < 0) {
		lderr(cct)
					<< "Error rolling back image: " << cpp_strerror(-r) << dendl;
		return r;
	}
...
public:
	...
	virtual void finish(int r) {
		...
		if (r < 0) {
			...
			comp->release();
			lderr(m_dest->cct)
						<< "error writing to destination image at offset "
								<< m_offset << ": " << cpp_strerror(r) << dendl;
		}
	}
private:
...
int copy(ImageCtx *src, ImageCtx *dest, ProgressContext &prog_ctx) {
	...
	if (dest_size < src_size) {
		lderr(cct)
					<< " src size " << src_size << " >= dest size " << dest_size
							<< dendl;
		return -EINVAL;
	}
	int r;
	...
	for (uint64_t offset = 0; offset < src_size; offset += period) {
		...
		if (r < 0) {
			...
			lderr(cct)
						<< "could not read from source image from " << offset
								<< " to " << offset + len << ": "
								<< cpp_strerror(r) << dendl;
			return r;
		}
...
int open_image(ImageCtx *ictx) {
	ldout(ictx->cct, 20)
				<< "open_image: ictx = " << ictx << " name = '" << ictx->name
						<< "' id = '" << ictx->id << "' snap_name = '"
						<< ictx->snap_name << "'" << dendl;
	int r = ictx->init();
	if (r < 0)
		goto err_close;
	...
	if (!ictx->read_only) {
		...
		if (r < 0) {
			lderr(ictx->cct)
						<< "error registering a watch: " << cpp_strerror(r)
								<< dendl;
			goto err_close;
		}
	}
	...
	err_close: close_image(ictx);
	return r;
}

void close_image(ImageCtx *ictx) {
	ldout(ictx->cct, 20)
				<< "close_image " << ictx << dendl;
...
int flatten(ImageCtx *ictx, ProgressContext &prog_ctx) {
	...
	for (uint64_t ono = 0; ono < overlap_objects; ono++) {
		...
		Striper::extent_to_file(cct, &ictx->layout, ono, 0, object_size,
				objectx);
		...
		if (r < 0) {
			lderr(cct)
						<< "failed to flatten object " << oid << dendl;
			goto err;
		}
	...
	if (r < 0) {
		lderr(cct)
					<< "failed to flatten at least one object: "
							<< cpp_strerror(r) << dendl;
		goto err;
	}
	...
	if (ictx->snaps.empty()) {
		ldout(cct, 2)
					<< "removing child from children list..." << dendl;
	...
	ldout(cct, 20)
				<< "finished flattening" << dendl;

	return 0;

	err: throttle.wait_for_ret();
	return r;
}

int list_lockers(ImageCtx *ictx, std::list<locker_t> *lockers,
bool *exclusive, string *tag) {
	ldout(ictx->cct, 20)
				<< "list_locks on image " << ictx << dendl;
...
int lock(ImageCtx *ictx, bool exclusive, const string& cookie,
		const string& tag) {
	ldout(ictx->cct, 20)
				<< "lock image " << ictx << " exclusive=" << exclusive
	...
	r = rados::cls::lock::lock(&ictx->md_ctx, ictx->header_oid, RBD_LOCK_NAME,
			exclusive ? LOCK_EXCLUSIVE : LOCK_SHARED, cookie, tag, "",
			utime_t(), 0);
	if (r < 0)
		return r;
	notify_change(ictx->md_ctx, ictx->header_oid, NULL, ictx);
...
int unlock(ImageCtx *ictx, const string& cookie) {
	ldout(ictx->cct, 20)
				<< "unlock image " << ictx << " cookie='" << cookie << "'"
						<< dendl;
...
int break_lock(ImageCtx *ictx, const string& client, const string& cookie) {
	ldout(ictx->cct, 20)
				<< "break_lock image " << ictx << " client='" << client
						<< "' cookie='" << cookie << "'" << dendl;
	...
	if (!lock_client.parse(client)) {
		lderr(ictx->cct)
					<< "Unable to parse client '" << client << "'" << dendl;
		return -EINVAL;
	}
	RWLock::RLocker locker(ictx->md_lock);
...
int diff_iterate(ImageCtx *ictx, const char *fromsnapname, uint64_t off,
		uint64_t len, int (*cb)(uint64_t, size_t, int, void *), void *arg) {
	utime_t start_time, elapsed;

	ldout(ictx->cct, 20)
				<< "diff_iterate " << ictx << " off = " << off << " len = "
						<< len << dendl;
	...
	ldout(ictx->cct, 20)
				<< "diff_iterate from " << from_snap_id << " to " << end_snap_id
						<< " size from " << from_size << " to " << end_size
						<< dendl;
	...
	if (from_snap_id == 0) {
		...
		if (ictx->parent && overlap > 0) {
			ldout(ictx->cct, 10)
						<< " first getting parent diff" << dendl;
			r = diff_iterate(ictx->parent, NULL, 0, overlap, simple_diff_cb,
					&parent_diff);
		}
		ictx->parent_lock.put_read();
		if (r < 0)
	...
	while (left > 0) {
		...
		for (map<object_t, vector<ObjectExtent> >::iterator p =
				object_extents.begin(); p != object_extents.end(); ++p) {
			ldout(ictx->cct, 20)
						<< "diff_iterate object " << p->first << dendl;
				...
				if (from_snap_id == 0 && !parent_diff.empty()) {
					...
					for (vector<ObjectExtent>::iterator q = p->second.begin();
						...
						for (vector<pair<uint64_t, uint64_t> >::iterator r =
							...
							ldout(ictx->cct, 20)
										<< " reporting parent overlap " << o
												<< dendl;
							for (interval_set<uint64_t>::iterator s = o.begin();
									s != o.end(); ++s) {
			...
			calc_snap_set_diff(ictx->cct, snap_set, from_snap_id, end_snap_id,
					&diff, &end_exists);
			ldout(ictx->cct, 20)
						<< "  diff " << diff << " end_exists=" << end_exists
								<< dendl;
			if (diff.empty())
				continue;
...
ssize_t read(ImageCtx *ictx,
		const vector<pair<uint64_t, uint64_t> >& image_extents, char *buf,
...
ssize_t handle_sparse_read(CephContext *cct, bufferlist data_bl,
		uint64_t block_ofs, const map<uint64_t, uint64_t> &data_map,
	...
	for (map<uint64_t, uint64_t>::const_iterator iter = data_map.begin();
		...
		ldout(cct, 10)
					<< "extent_ofs=" << extent_ofs << " extent_len="
							<< extent_len << dendl;
		ldout(cct, 10)
					<< "block_ofs=" << block_ofs << dendl;

					/* a hole? */
		if (extent_ofs > block_ofs) {
			uint64_t gap = extent_ofs - block_ofs;
			ldout(cct, 10)
						<< "<1>zeroing " << buf_ofs << "~" << gap << dendl;
	...
	if (buf_left > 0) {
		ldout(cct, 10)
					<< "<3>zeroing " << buf_ofs << "~" << buf_left << dendl;
...
int aio_flush(ImageCtx *ictx, AioCompletion *c) {
	CephContext *cct = ictx->cct;
	ldout(cct, 20)
				<< "aio_flush " << ictx << " completion " << c << dendl;
...
int aio_write(ImageCtx *ictx, uint64_t off, size_t len, const char *buf,
	...
	CephContext *cct = ictx->cct;
	ldout(cct, 20)
				<< "aio_write " << ictx << " off = " << off << " len = " << len
						<< " buf = " << (void*) buf << dendl;
	...
	for (vector<ObjectExtent>::iterator p = extents.begin(); p != extents.end();
			++p) {
		ldout(cct, 20)
					<< " oid " << p->oid << " " << p->offset << "~" << p->length
							<< " from " << p->buffer_extents << dendl;
					// assemble extent
		bufferlist bl;
		for (vector<pair<uint64_t, uint64_t> >::iterator q =
				p->buffer_extents.begin(); q != p->buffer_extents.end(); ++q) {
		...
		if (ictx->object_cacher) {
			...
			vector<pair<uint64_t, uint64_t> > objectx;
			Striper::extent_to_file(ictx->cct, &ictx->layout, p->objectno, 0,
					ictx->layout.fl_object_size, objectx);
			uint64_t object_overlap = ictx->prune_parent_extents(objectx,
					overlap);

			AioWrite *req = new AioWrite(ictx, p->oid.name, p->objectno,
					p->offset, objectx, object_overlap, bl, snapc, snap_id,
					req_comp, hint);
			c->add_request();
			r = req->send();
			if (r < 0)
	...
	done: c->finish_adding_requests(ictx->cct);
	c->put();
	...
	for (vector<ObjectExtent>::iterator p = extents.begin(); p != extents.end();
		...
		if (p->offset == 0 && p->length == ictx->layout.fl_object_size) {
			req = new AioRemove(ictx, p->oid.name, p->objectno, objectx,
					object_overlap, snapc, snap_id, req_comp);
		} else if (p->offset + p->length == ictx->layout.fl_object_size) {
			req = new AioTruncate(ictx, p->oid.name, p->objectno, p->offset,
					objectx, object_overlap, snapc, snap_id, req_comp);
		} else {
			req = new AioZero(ictx, p->oid.name, p->objectno, p->offset,
					p->length, objectx, object_overlap, snapc, snap_id,
					req_comp);
		}
...
int aio_read(ImageCtx *ictx, uint64_t off, size_t len, char *buf,
...
static void readahead(ImageCtx *ictx,
	...
	for (vector<pair<uint64_t, uint64_t> >::const_iterator p =
			image_extents.begin(); p != image_extents.end(); ++p) {
		total_bytes += p->second;
	}
	ictx->md_lock.get_write();
	bool abort = conf->rbd_readahead_disable_after_bytes != 0
			&& ictx->total_bytes_read
					> (uint64_t) conf->rbd_readahead_disable_after_bytes;
	...
	pair<uint64_t, uint64_t> readahead_extent = ictx->readahead.update(
			image_extents, image_size);
	uint64_t readahead_offset = readahead_extent.first;
	uint64_t readahead_length = readahead_extent.second;

	if (readahead_length > 0) {
		ldout(ictx->cct, 20)
					<< "(readahead logical) " << readahead_offset << "~"
		...
		for (map<object_t, vector<ObjectExtent> >::iterator p =
				readahead_object_extents.begin();
				p != readahead_object_extents.end(); ++p) {
			for (vector<ObjectExtent>::iterator q = p->second.begin();
					q != p->second.end(); ++q) {
				ldout(ictx->cct, 20)
							<< "(readahead) oid " << q->oid << " " << q->offset
									<< "~" << q->length << dendl;
				...
				struct C_RBD_Readahead: public Context {
					...
					uint64_t offset;
					uint64_t length;
					C_RBD_Readahead(ImageCtx *ictx, object_t oid,
							uint64_t offset, uint64_t length) :
							ictx(ictx), oid(oid), offset(offset), length(length) {
					}
					void finish(int r) {
						ldout(ictx->cct, 20)
									<< "C_RBD_Readahead on " << oid << ": "
											<< offset << "+" << length << dendl;
						ictx->readahead.dec_pending();
					}
				};

				Context *req_comp = new C_RBD_Readahead(ictx, q->oid, q->offset,
						q->length);
				ictx->readahead.inc_pending();
				ictx->aio_read_from_cache(q->oid, NULL, q->length, q->offset,
						req_comp);
			}
		}
...
int aio_read(ImageCtx *ictx,
		const vector<pair<uint64_t, uint64_t> >& image_extents, char *buf,
		bufferlist *pbl, AioCompletion *c, char* hint) {
	ldout(ictx->cct, 20)
				<< "aio_read " << ictx << " completion " << c << " "
						<< image_extents << dendl;
	...
	for (map<object_t, vector<ObjectExtent> >::iterator p =
		...
		for (vector<ObjectExtent>::iterator q = p->second.begin();
			...
			AioRead *req = new AioRead(ictx, q->oid.name, q->objectno,
					q->offset, q->length, q->buffer_extents, snap_id, true,
					req_comp, hint);
			...
			if (ictx->object_cacher) {
				...
				ictx->aio_read_from_cache(q->oid, &req->data(), q->length,
						q->offset, cache_comp, hint);
			} else {
				r = req->send();
				if (r < 0 && r == -ENOENT)
					r = 0;
