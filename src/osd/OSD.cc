...
OSDService::OSDService(OSD *osd) :
		osd(osd), cct(osd->cct), whoami(osd->whoami), store(osd->store), log_client(
				osd->log_client), clog(osd->clog), pg_recovery_stats(
				osd->pg_recovery_stats), infos_oid(OSD::make_infos_oid()), cluster_messenger(
				osd->cluster_messenger), client_messenger(
				osd->client_messenger), logger(osd->logger), recoverystate_perf(
				osd->recoverystate_perf), monc(osd->monc), op_wq(
				osd->op_shardedwq), peering_wq(osd->peering_wq), recovery_wq(
				osd->recovery_wq), snap_trim_wq(osd->snap_trim_wq), scrub_wq(
				osd->scrub_wq), scrub_finalize_wq(osd->scrub_finalize_wq), rep_scrub_wq(
				osd->rep_scrub_wq), recovery_gen_wq("recovery_gen_wq",
				cct->_conf->osd_recovery_thread_timeout, &osd->recovery_tp), op_gen_wq(
				"op_gen_wq", cct->_conf->osd_recovery_thread_timeout,
				&osd->osd_tp), class_handler(osd->class_handler), pg_epoch_lock(
				"OSDService::pg_epoch_lock"), publish_lock(
				"OSDService::publish_lock"), pre_publish_lock(
				"OSDService::pre_publish_lock"), peer_map_epoch_lock(
				"OSDService::peer_map_epoch_lock"), sched_scrub_lock(
				"OSDService::sched_scrub_lock"), scrubs_pending(0), scrubs_active(
				0), agent_lock("OSD::agent_lock"), agent_valid_iterator(false), agent_ops(
				0), agent_active(true), agent_thread(this), agent_stop_flag(
				false), agent_timer_lock("OSD::agent_timer_lock"), agent_timer(
				osd->client_messenger->cct, agent_timer_lock), objecter(
				new Objecter(osd->client_messenger->cct,
						osd->objecter_messenger, osd->monc, 0, 0)), objecter_finisher(
				osd->client_messenger->cct), watch_lock("OSD::watch_lock"), watch_timer(
				osd->client_messenger->cct, watch_lock), next_notif_id(0), backfill_request_lock(
				"OSD::backfill_request_lock"), backfill_request_timer(cct,
				backfill_request_lock, false), last_tid(0), tid_lock(
				"OSDService::tid_lock"), reserver_finisher(cct), local_reserver(
				&reserver_finisher, cct->_conf->osd_max_backfills,
				cct->_conf->osd_min_recovery_priority), remote_reserver(
				&reserver_finisher, cct->_conf->osd_max_backfills,
				cct->_conf->osd_min_recovery_priority), pg_temp_lock(
				"OSDService::pg_temp_lock"), map_cache_lock(
				"OSDService::map_lock"), map_cache(cct,
				cct->_conf->osd_map_cache_size), map_bl_cache(
				cct->_conf->osd_map_cache_size), map_bl_inc_cache(
				cct->_conf->osd_map_cache_size), in_progress_split_lock(
				"OSDService::in_progress_split_lock"), stat_lock(
				"OSD::stat_lock"), full_status_lock(
				"OSDService::full_status_lock"), cur_state(NONE), last_msg(0), cur_ratio(
				0), epoch_lock("OSDService::epoch_lock"), boot_epoch(0), up_epoch(
				0), bind_epoch(0), is_stopping_lock(
				"OSDService::is_stopping_lock"), state(NOT_STOPPING)
#ifdef PG_DEBUG_REFS
...
void OSDService::_start_split(spg_t parent, const set<spg_t> &children) {
	for (set<spg_t>::const_iterator i = children.begin(); i != children.end();
			++i) {
		dout(10)
					<< __func__ << ": Starting split on pg " << *i
							<< ", parent=" << parent << dendl;
...
void OSDService::mark_split_in_progress(spg_t parent,
		const set<spg_t> &children) {
...
void OSDService::_cancel_pending_splits_for_parent(spg_t parent) {
	...
	for (set<spg_t>::iterator i = piter->second.begin();
			i != piter->second.end(); ++i) {
		assert(pending_splits.count(*i));
		assert(!in_progress_splits.count(*i));
		pending_splits.erase(*i);
		dout(10)
					<< __func__ << ": Completing split on pg " << *i
...
void OSDService::_maybe_split_pgid(OSDMapRef old_map, OSDMapRef new_map,
		spg_t pgid) {
	...
	if (pgid.ps() < static_cast<unsigned>(old_map->get_pg_num(pgid.pool()))) {
		...
		assert(
				pgid.ps()
						< static_cast<unsigned>(new_map->get_pg_num(pgid.pool())));
	}
}

void OSDService::init_splits_between(spg_t pgid, OSDMapRef frommap,
		OSDMapRef tomap) {
	// First, check whether we can avoid this potentially expensive check
	if (tomap->have_pg_pool(pgid.pool())
			&& pgid.is_split(frommap->get_pg_num(pgid.pool()),
		...
		for (epoch_t e = frommap->get_epoch() + 1; e <= tomap->get_epoch();
				++e) {
			OSDMapRef nextmap(try_get_map(e));
			if (!nextmap)
				continue;
			set<spg_t> even_newer_pgs; // pgs added in this loop
			for (set<spg_t>::iterator i = new_pgs.begin(); i != new_pgs.end();
					++i) {
				set<spg_t> split_pgs;
				if (i->is_split(curmap->get_pg_num(i->pool()),
						nextmap->get_pg_num(i->pool()), &split_pgs)) {
...
void OSDService::expand_pg_num(OSDMapRef old_map, OSDMapRef new_map) {
	Mutex::Locker l(in_progress_split_lock);
...
bool OSDService::splitting(spg_t pgid) {
	Mutex::Locker l(in_progress_split_lock);
	return in_progress_splits.count(pgid) || pending_splits.count(pgid);
}

void OSDService::complete_split(const set<spg_t> &pgs) {
	Mutex::Locker l(in_progress_split_lock);
	for (set<spg_t>::const_iterator i = pgs.begin(); i != pgs.end(); ++i) {
		dout(10)
					<< __func__ << ": Completing split on pg " << *i << dendl;
		assert(!pending_splits.count(*i));
		assert(in_progress_splits.count(*i));
		in_progress_splits.erase(*i);
	}
...
void OSDService::agent_entry() {
	dout(10)
				<< __func__ << " start" << dendl;
	agent_lock.Lock();

	while (!agent_stop_flag) {
		if (agent_queue.empty()) {
			dout(20)
						<< __func__ << " empty queue" << dendl;
		...
		dout(10)
					<< __func__ << " tiers " << agent_queue.size()
							<< ", top is " << level << " with pgs "
							<< top.size() << ", ops " << agent_ops << "/"
							<< g_conf->osd_agent_max_ops
							<< (agent_active ? " active" : " NOT ACTIVE")
							<< dendl;
		dout(20)
					<< __func__ << " oids " << agent_oids << dendl;
		...
		if (!pg->agent_work(max)) {
			dout(10)
						<< __func__ << " " << *pg
								<< " no agent_work, delay for "
								<< g_conf->osd_agent_delay_time << " seconds"
								<< dendl;
...
void OSDService::agent_stop() {
	{
		...
		if (!agent_queue.empty()) {
			set<PGRef>& top = agent_queue.rbegin()->second;
			derr
						<< "agent queue not empty, for example "
								<< (*top.begin())->info.pgid << dendl;assert(
					0 == "agent queue not empty");
		}
...
void OSDService::check_nearfull_warning(const osd_stat_t &osd_stat) {
	...
	last_msg = now;
	if (cur_state == FULL)
		clog->error() << "OSD full dropping all updates " << (int) (ratio * 100)
				<< "% full";
	else
...
void OSDService::update_osd_stat(vector<int>& hb_peers) {
	...
	dout(20)
				<< "update_osd_stat " << osd_stat << dendl;
}
...
void OSDService::send_message_osd_cluster(int peer, Message *m,
	...
	Connection *peer_con =
			osd->cluster_messenger->get_connection(peer_inst).get();
...
ConnectionRef OSDService::get_con_osd_cluster(int peer, epoch_t from_epoch) {
	...
	ConnectionRef con = osd->cluster_messenger->get_connection(
			next_map->get_cluster_inst(peer));
	release_map(next_map);
	return con;
}

pair<ConnectionRef, ConnectionRef> OSDService::get_con_osd_hb(int peer,
		epoch_t from_epoch) {
	OSDMapRef next_map = get_nextmap_reserved();
	// service map is always newer/newest
	assert(from_epoch <= next_map->get_epoch());
...
void OSDService::send_pg_temp() {
	...
	if (pg_temp_wanted.empty())
		return;
	dout(10)
				<< "send_pg_temp " << pg_temp_wanted << dendl;
...
epoch_t OSDService::note_peer_epoch(int peer, epoch_t e) {
	...
	if (p != peer_map_epoch.end()) {
		if (p->second < e) {
			dout(10)
						<< "note_peer_epoch osd." << peer << " has " << e
								<< dendl;
			p->second = e;
		} else {
			dout(30)
						<< "note_peer_epoch osd." << peer << " has "
								<< p->second << " >= " << e << dendl;
		}
		return p->second;
	} else {
		dout(10)
					<< "note_peer_epoch osd." << peer << " now has " << e
							<< dendl;
...
void OSDService::forget_peer_epoch(int peer, epoch_t as_of) {
	...
	if (p != peer_map_epoch.end()) {
		if (p->second <= as_of) {
			dout(10)
						<< "forget_peer_epoch osd." << peer << " as_of "
								<< as_of << " had " << p->second << dendl;
...
bool OSDService::should_share_map(entity_name_t name, Connection *con,
		epoch_t epoch, OSDMapRef& osdmap, const epoch_t *sent_epoch_p) {
	bool should_send = false;
	dout(20)
				<< "should_share_map " << name << " " << con->get_peer_addr()
						<< " " << epoch << dendl;
	...
	if (name.is_client()) {
		...
		if (message_sendmap && sent_epoch_p) {
			dout(20)
						<< "client session last_sent_epoch: " << *sent_epoch_p
								<< " versus osdmap epoch "
	...
	if (con->get_messenger() == osd->cluster_messenger
			...
			&& (osdmap->get_cluster_addr(name.num()) == con->get_peer_addr()
					|| osdmap->get_hb_back_addr(name.num())
							== con->get_peer_addr())) {
		// remember
...
void OSDService::share_map(entity_name_t name, Connection *con, epoch_t epoch,
		OSDMapRef& osdmap, epoch_t *sent_epoch_p) {
	dout(20)
				<< "share_map " << name << " " << con->get_peer_addr() << " "
						<< epoch << dendl;
	...
	bool want_shared = should_share_map(name, con, epoch, osdmap, sent_epoch_p);

	if (want_shared) {
		if (name.is_client()) {
			dout(10)
						<< name << " has old map " << epoch << " < "
								<< osdmap->get_epoch() << dendl;
...
void OSDService::share_map_peer(int peer, Connection *con, OSDMapRef map) {
	...
	if (pe) {
		if (pe < map->get_epoch()) {
			...
			dout(20)
						<< "share_map_peer " << con << " already has epoch "
								<< pe << dendl;
	} else {
		dout(20)
					<< "share_map_peer " << con
...
bool OSDService::inc_scrubs_pending() {
	...
	if (scrubs_pending + scrubs_active < cct->_conf->osd_max_scrubs) {
		dout(20)
					<< "inc_scrubs_pending " << scrubs_pending << " -> "
							<< (scrubs_pending + 1) << " (max "
							<< cct->_conf->osd_max_scrubs << ", active "
							<< scrubs_active << ")" << dendl;
		result = true;
		++scrubs_pending;
	} else {
...
void OSDService::dec_scrubs_pending() {
	sched_scrub_lock.Lock();
	dout(20)
				<< "dec_scrubs_pending " << scrubs_pending << " -> "
						<< (scrubs_pending - 1) << " (max "
						<< cct->_conf->osd_max_scrubs << ", active "
						<< scrubs_active << ")" << dendl;
	--scrubs_pending;
...
void OSDService::inc_scrubs_active(bool reserved) {
	...
	if (reserved) {
		--(scrubs_pending);
		dout(20)
					<< "inc_scrubs_active " << (scrubs_active - 1) << " -> "
							<< scrubs_active << " (max "
							<< cct->_conf->osd_max_scrubs << ", pending "
							<< (scrubs_pending + 1) << " -> " << scrubs_pending
							<< ")" << dendl;assert(scrubs_pending >= 0);
	} else {
		dout(20)
					<< "inc_scrubs_active " << (scrubs_active - 1) << " -> "
							<< scrubs_active << " (max "
							<< cct->_conf->osd_max_scrubs << ", pending "
							<< scrubs_pending << ")" << dendl;
	}
	sched_scrub_lock.Unlock();
}
...
void OSDService::dec_scrubs_active() {
	sched_scrub_lock.Lock();
	dout(20)
				<< "dec_scrubs_active " << scrubs_active << " -> "
						<< (scrubs_active - 1) << " (max "
						<< cct->_conf->osd_max_scrubs << ", pending "
						<< scrubs_pending << ")" << dendl;
	--scrubs_active;
...
void OSDService::set_epochs(const epoch_t *_boot_epoch,
		const epoch_t *_up_epoch, const epoch_t *_bind_epoch) {
	Mutex::Locker l(epoch_lock);
	if (_boot_epoch) {
...
bool OSDService::prepare_to_stop() {
	...
	if (osdmap && osdmap->is_up(whoami)) {
		dout(0)
					<< __func__ << " telling mon we are shutting down" << dendl;
		state = PREPARING_TO_STOP;
		monc->send_mon_message(
				new MOSDMarkMeDown(monc->get_fsid(), osdmap->get_inst(whoami),
						osdmap->get_epoch(),
						true  // request ack
						));
		...
		while ((ceph_clock_now(cct) < timeout) && (state != STOPPING)) {
			is_stopping_cond.WaitUntil(is_stopping_lock, timeout);
		}
	}
	dout(0)
				<< __func__ << " starting shutdown" << dendl;
	state = STOPPING;
	return true;
}
...
MOSDMap *OSDService::build_incremental_map_msg(epoch_t since, epoch_t to,
	...
	for (epoch_t e = to; e > since; e--) {
		...
		if (e > m->oldest_map && get_inc_map_bl(e, bl)) {
			...
			derr
						<< "since " << since << " to " << to << " oldest "
								<< m->oldest_map << " newest " << m->newest_map
								<< dendl;
			m->put();
			m = NULL;
			break;
...
void OSDService::send_incremental_map(epoch_t since, Connection *con,
	...
	dout(10)
				<< "send_incremental_map " << since << " -> " << to << " to "
						<< con << " " << con->get_peer_addr() << dendl;
...
bool OSDService::_get_map_bl(epoch_t e, bufferlist& bl) {
	...
	if (found)
		return true;
	found = store->read(META_COLL, OSD::get_osdmap_pobject_name(e), 0, 0, bl)
			>= 0;
	if (found)
		_add_map_bl(e, bl);
	return found;
...
bool OSDService::get_inc_map_bl(epoch_t e, bufferlist& bl) {
	...
	if (found)
		return true;
	found = store->read(META_COLL, OSD::get_inc_osdmap_pobject_name(e), 0, 0,
			bl) >= 0;
	if (found)
...
OSDMapRef OSDService::try_get_map(epoch_t epoch) {
	...
	if (epoch > 0) {
		dout(20)
					<< "get_map " << epoch << " - loading and decoding " << map
							<< dendl;
		bufferlist bl;
		...
		dout(20)
					<< "get_map " << epoch << " - return initial " << map
							<< dendl;
	}
	return _add_map(map);
}
...
bool OSDService::queue_for_recovery(PG *pg) {
	...
	if (b)
		dout(10)
					<< "queue_for_recovery queued " << *pg << dendl;
	else
		dout(10)
					<< "queue_for_recovery already queued " << *pg << dendl;
	return b;
}
...
void OSDService::handle_misdirected_op(PG *pg, OpRequestRef op) {
	...
	if (pg->is_ec_pg()) {
		...
		if (!opmap) {
			dout(7)
						<< __func__ << ": " << *pg << " no longer have map for "
		...
		if (opmap->get_primary_shard(_pgid, &pgid)
			...
			dout(7)
						<< __func__ << ": " << *pg << " primary changed since "
								<< m->get_map_epoch() << ", dropping" << dendl;
			return;
		}
	}
	...
	dout(7)
				<< *pg << " misdirected op in " << m->get_map_epoch() << dendl;
	clog->warn() << m->get_source_inst() << " misdirected " << m->get_reqid()
			<< " pg " << m->get_pg() << " to osd." << whoami << " not "
			<< pg->acting << " in e" << m->get_map_epoch() << "/"
			<< osdmap->get_epoch() << "\n";
...
int OSD::mkfs(CephContext *cct, ObjectStore *store, const string &dev,
	...
	try {
		...
		if (cct->_conf->osd_age_time != 0) {
			if (cct->_conf->osd_age_time >= 0) {
				dout(0)
							<< "aging..." << dendl;
				Ager ager(cct, store);
				ager.age(cct->_conf->osd_age_time, cct->_conf->osd_age,
						cct->_conf->osd_age - .05, 50000,
		...
		if (ret >= 0) {
			dout(0)
						<< " have superblock" << dendl;
			if (whoami != sb.whoami) {
				derr
							<< "provided osd id " << whoami
									<< " != superblock's " << sb.whoami << dendl;
				ret = -EINVAL;
				goto umount_store;
			}
			...
			if (cct->_conf->osd_auto_weight) {
				...
				bl.push_back(bp);
				dout(0)
							<< "testing disk bandwidth..." << dendl;
				utime_t start = ceph_clock_now(cct);
				object_t oid("disk_bw_test");
				for (int i = 0; i < 1000; i++) {
					ObjectStore::Transaction *t = new ObjectStore::Transaction;
					t->write(META_COLL, hobject_t(sobject_t(oid, 0)),
							i * bl.length(), bl.length(), bl);
				...
				if (ret) {
					derr
										<< "OSD::mkfs: error while benchmarking: apply_transaction returned "
										<< ret << dendl;
					goto umount_store;
				}
			...
			if (ret) {
				derr
									<< "OSD::mkfs: error while writing OSD_SUPERBLOCK_POBJECT: "
									<< "apply_transaction returned " << ret
									<< dendl;
				goto umount_store;
			}
		}
		...
		derr
					<< "OSD::mkfs: caught exception " << se.what() << dendl;
		ret = 1000;
	} catch (...) {
		derr
					<< "OSD::mkfs: caught unknown exception." << dendl;
		ret = 1000;
	}

	umount_store: store->umount();
	free_store: delete store;
	return ret;
}

int OSD::write_meta(ObjectStore *store, uuid_d& cluster_fsid, uuid_d& osd_fsid,
		int whoami) {
	char val[80];
	int r;
...
OSD::OSD(CephContext *cct_, ObjectStore *store_, int id,
		...
		Messenger *hb_back_serverm, Messenger *osdc_messenger, MonClient *mc,
		const std::string &dev, const std::string &jdev) :
		Dispatcher(cct_), osd_lock("OSD::osd_lock"), tick_timer(cct, osd_lock), authorize_handler_cluster_registry(
				new AuthAuthorizeHandlerRegistry(cct,
						cct->_conf->auth_supported.length() ?
								cct->_conf->auth_supported :
								cct->_conf->auth_cluster_required)), authorize_handler_service_registry(
				new AuthAuthorizeHandlerRegistry(cct,
						cct->_conf->auth_supported.length() ?
								cct->_conf->auth_supported :
								cct->_conf->auth_service_required)), cluster_messenger(
				internal_messenger), client_messenger(external_messenger), objecter_messenger(
				osdc_messenger), monc(mc), logger(NULL), recoverystate_perf(
				NULL), store(store_), log_client(cct, client_messenger,
				&mc->monmap, LogClient::NO_FLAGS), clog(
				log_client.create_channel()), whoami(id), dev_path(dev), journal_path(
				jdev), dispatch_running(false), asok_hook(NULL), osd_compat(
				get_osd_compat_set()), state(STATE_INITIALIZING), osd_tp(cct,
				"OSD::osd_tp", cct->_conf->osd_op_threads, "osd_op_threads"), osd_op_tp(
				cct, "OSD::osd_op_tp",
				cct->_conf->osd_op_num_threads_per_shard
						* cct->_conf->osd_op_num_shards), recovery_tp(cct,
				"OSD::recovery_tp", cct->_conf->osd_recovery_threads,
				"osd_recovery_threads"), disk_tp(cct, "OSD::disk_tp",
				cct->_conf->osd_disk_threads, "osd_disk_threads"), command_tp(
				cct, "OSD::command_tp", 1), paused_recovery(false), session_waiting_lock(
				"OSD::session_waiting_lock"), heartbeat_lock(
				"OSD::heartbeat_lock"), heartbeat_stop(false), heartbeat_update_lock(
				"OSD::heartbeat_update_lock"), heartbeat_need_update(true), heartbeat_epoch(
				0), hbclient_messenger(hb_clientm), hb_front_server_messenger(
				hb_front_serverm), hb_back_server_messenger(hb_back_serverm), heartbeat_thread(
				this), heartbeat_dispatcher(this), finished_lock(
				"OSD::finished_lock"), op_tracker(cct,
				cct->_conf->osd_enable_op_tracker,
				cct->_conf->osd_num_op_tracker_shard), test_ops_hook(NULL), op_shardedwq(
				cct->_conf->osd_op_num_shards, this,
				cct->_conf->osd_op_thread_timeout, &osd_op_tp), peering_wq(this,
				cct->_conf->osd_op_thread_timeout, &osd_tp), map_lock(
				"OSD::map_lock"), pg_map_lock("OSD::pg_map_lock"), debug_drop_pg_create_probability(
				cct->_conf->osd_debug_drop_pg_create_probability), debug_drop_pg_create_duration(
				cct->_conf->osd_debug_drop_pg_create_duration), debug_drop_pg_create_left(
				-1), outstanding_pg_stats(false), timeout_mon_on_pg_stats(true), up_thru_wanted(
				0), up_thru_pending(0), pg_stat_queue_lock(
				"OSD::pg_stat_queue_lock"), osd_stat_updated(false), pg_stat_tid(
				0), pg_stat_tid_flushed(0), command_wq(this,
				cct->_conf->osd_command_thread_timeout, &command_tp), recovery_ops_active(
				0), recovery_wq(this, cct->_conf->osd_recovery_thread_timeout,
				&recovery_tp), replay_queue_lock("OSD::replay_queue_lock"), snap_trim_wq(
				this, cct->_conf->osd_snap_trim_thread_timeout, &disk_tp), scrub_wq(
				this, cct->_conf->osd_scrub_thread_timeout, &disk_tp), scrub_finalize_wq(
				cct->_conf->osd_scrub_finalize_thread_timeout, &osd_tp), rep_scrub_wq(
				this, cct->_conf->osd_scrub_thread_timeout, &disk_tp), remove_wq(
				store, cct->_conf->osd_remove_thread_timeout, &disk_tp), next_removal_seq(
				0), service(this) {
...
void OSD::handle_signal(int signum) {
	...
	derr
				<< "*** Got signal " << sys_siglist[signum] << " ***" << dendl;
				//suicide(128 + signum);
	shutdown();
}
...
int OSD::pre_init() {
	...
	if (store->test_mount_in_use()) {
		derr
					<< "OSD::pre_init: object store '" << dev_path << "' is "
							<< "currently in use. (Is ceph-osd already running?)"
							<< dendl;
		return -EBUSY;
	}
...
int OSD::init() {
	...
	if (osd_compat.compare(superblock.compat_features) < 0) {
		derr
					<< "The disk uses features unsupported by the executable."
							<< dendl;
		derr
					<< " ondisk features " << superblock.compat_features
							<< dendl;
		derr
					<< " daemon features " << osd_compat << dendl;

		if (osd_compat.writeable(superblock.compat_features)) {
			CompatSet diff = osd_compat.unsupported(superblock.compat_features);
			derr
						<< "it is still writeable, though. Missing features: "
								<< diff << dendl;
			r = -EOPNOTSUPP;
			goto out;
		} else {
	...
	if (whoami != superblock.whoami) {
		derr
					<< "OSD::init: superblock says osd" << superblock.whoami
	...
	if (superblock.compat_features.merge(initial)) {
		...
		// do anything else
		dout(5)
					<< "Upgrading superblock adding: " << diff << dendl;
		ObjectStore::Transaction t;
		write_superblock(t);
	...
	if (!store->exists(META_COLL, service.infos_oid)) {
		dout(10)
					<< "init creating/touching infos object" << dendl;
		ObjectStore::Transaction t;
	...
	if (!store->exists(META_COLL, OSD::make_snapmapper_oid())) {
		dout(10)
					<< "init creating/touching snapmapper object" << dendl;
		ObjectStore::Transaction t;
	...
	dout(2)
				<< "superblock: i am osd." << superblock.whoami << dendl;
	...
	// tick
	tick_timer.add_event_after(cct->_conf->osd_heartbeat_interval,
			new C_Tick(this));
	...
	dout(10)
				<< "ensuring pgs have consumed prior maps" << dendl;
	consume_map();
	peering_wq.drain();

	dout(0)
				<< "done with init, starting boot process" << dendl;
	set_state(STATE_BOOTING);
	start_boot();

	return 0;
	monout: monc->shutdown();
...
void OSD::final_init() {
	...
	r = admin_socket->register_command("dump_op_pq_state", "dump_op_pq_state",
			asok_hook, "dump op priority queue state");
	assert(r == 0);
	r = admin_socket->register_command("dump_blacklist", "dump_blacklist",
			asok_hook, "dump blacklisted clients and times");
	assert(r == 0);
	r = admin_socket->register_command("dump_watchers", "dump_watchers",
			asok_hook, "show clients which have active watches,"
					" and on which objects");
	assert(r == 0);
	r = admin_socket->register_command("dump_reservations", "dump_reservations",
			asok_hook, "show recovery reservations");
	assert(r == 0);

	test_ops_hook = new TestOpsSocketHook(&(this->service), this->store);
	// Note: pools are CephString instead of CephPoolname because
	// these commands traditionally support both pool names and numbers
	r = admin_socket->register_command("setomapval", "setomapval "
			"name=pool,type=CephString "
			"name=objname,type=CephObjectname "
			"name=key,type=CephString "
			"name=val,type=CephString", test_ops_hook, "set omap key");
	assert(r == 0);
	r = admin_socket->register_command("rmomapkey", "rmomapkey "
			"name=pool,type=CephString "
			"name=objname,type=CephObjectname "
			"name=key,type=CephString", test_ops_hook, "remove omap key");
	assert(r == 0);
	r = admin_socket->register_command("setomapheader", "setomapheader "
			"name=pool,type=CephString "
			"name=objname,type=CephObjectname "
			"name=header,type=CephString", test_ops_hook, "set omap header");
	assert(r == 0);

	r = admin_socket->register_command("getomap", "getomap "
			"name=pool,type=CephString "
			"name=objname,type=CephObjectname", test_ops_hook,
			"output entire object map");
	assert(r == 0);

	r = admin_socket->register_command("truncobj", "truncobj "
			"name=pool,type=CephString "
			"name=objname,type=CephObjectname "
			"name=len,type=CephInt", test_ops_hook,
	...
	r = admin_socket->register_command("injectmdataerr", "injectmdataerr "
			"name=pool,type=CephString "
			"name=objname,type=CephObjectname", test_ops_hook,
...
void OSD::create_logger() {
	...
	osd_plb.add_u64_counter(l_osd_waiting_for_map, "messages_delayed_for_map"); // dup osdmap epochs
...
void OSD::create_recoverystate_perf() {
	dout(10)
				<< "create_recoverystate_perf" << dendl;
	...
	rs_perf.add_time_avg(rs_repnotrecovering_latency,
	...
	rs_perf.add_time_avg(rs_waitactingchange_latency,
...
void OSD::suicide(int exitcode) {
	if (cct->_conf->filestore_blackhole) {
		derr
					<< " filestore_blackhole=true, doing abbreviated shutdown"
							<< dendl;
		_exit(exitcode);
	}
...
int OSD::shutdown() {
	...
	osd_tp.stop();
	dout(10)
				<< "osd tp stopped" << dendl;

	osd_op_tp.drain();
	osd_op_tp.stop();
	dout(10)
				<< "op sharded tp stopped" << dendl;

	command_tp.drain();
	command_tp.stop();
	dout(10)
				<< "command tp stopped" << dendl;

	disk_tp.drain();
	disk_tp.stop();
	dout(10)
				<< "disk tp paused (new)" << dendl;

	dout(10)
				<< "stopping agent" << dendl;
	service.agent_stop();
	...
	// note unmount epoch
	dout(10)
				<< "noting clean unmount in epoch " << osdmap->get_epoch()
						<< dendl;
...
#endif
	{
		...
		for (ceph::unordered_map<spg_t, PG*>::iterator p = pg_map.begin();
			...
			if (p->second->ref.read() != 1) {
				derr
							<< "pgid " << p->first << " has ref count of "
									<< p->second->ref.read() << dendl;assert(0);
			}
			p->second->unlock();
			p->second->put("PGMap");
		}
...
void OSD::write_superblock(ObjectStore::Transaction& t) {
	dout(10)
				<< "write_superblock " << superblock << dendl;
...
int OSD::read_superblock() {
	...
	dout(10)
				<< "read_superblock " << superblock << dendl;
...
PGPool OSD::_get_pool(int id, OSDMapRef createmap) {
	...
	dout(10)
				<< "_get_pool " << p.id << dendl;
	return p;
}

PG *OSD::_open_lock_pg(OSDMapRef createmap, spg_t pgid, bool no_lockdep_check,
		bool hold_map_lock) {
...
void OSD::add_newly_split_pg(PG *pg, PG::RecoveryCtx *rctx) {
	...
	dout(10)
				<< "Adding newly split pg " << *pg << dendl;
	...
	if (to_wake != peering_wait_for_split.end()) {
		for (list<PG::CephPeeringEvtRef>::iterator i = to_wake->second.begin();
				i != to_wake->second.end(); ++i) {
...
OSD::res_result OSD::_try_resurrect_pg(OSDMapRef curmap, spg_t pgid,
		spg_t *resurrected, PGRef *old_pg_state) {
	assert(resurrected);
	assert(old_pg_state);
	...
	if (cur == pgid) {
		if (df->try_stop_deletion()) {
			dout(10)
						<< __func__ << ": halted deletion on pg " << pgid
								<< dendl;
			*resurrected = cur;
			*old_pg_state = df->old_pg_state;
			...
			curmap->get_pg_num(cur.pool()), &children)
			&& children.count(pgid)) {
		if (df->try_stop_deletion()) {
			dout(10)
						<< __func__ << ": halted deletion on ancestor pg "
								<< pgid << dendl;
			*resurrected = cur;
			*old_pg_state = df->old_pg_state;
...
PG *OSD::_create_lock_pg(OSDMapRef createmap, spg_t pgid,
bool newly_created,
bool hold_map_lock,
bool backfill, int role, vector<int>& up, int up_primary, vector<int>& acting,
		int acting_primary, pg_history_t history, pg_interval_map_t& pi,
		ObjectStore::Transaction& t) {
	assert(osd_lock.is_locked());
	dout(20)
				<< "_create_lock_pg pgid " << pgid << dendl;

	PG *pg = _open_lock_pg(createmap, pgid, true, hold_map_lock);

	service.init_splits_between(pgid, pg->get_osdmap(), service.get_osdmap());

	pg->init(role, up, up_primary, acting, acting_primary, history, pi,
			backfill, &t);

	dout(7)
				<< "_create_lock_pg " << *pg << dendl;
	return pg;
}

PG *OSD::get_pg_or_queue_for_pg(const spg_t& pgid, OpRequestRef& op) {
	Session *session =
			static_cast<Session*>(op->get_req()->get_connection()->get_priv());
	assert(session);
...
void OSD::load_pgs() {
	...
	if (r < 0) {
		derr
					<< "failed to list pgs: " << cpp_strerror(-r) << dendl;
	}
	...
	for (vector<coll_t>::iterator it = ls.begin(); it != ls.end(); ++it) {
		...
		if (it->is_temp(pgid) || it->is_removal(&seq, &pgid)
				|| store->collection_getattr(*it, "remove", &val, 1) > 0) {
			dout(10)
						<< "load_pgs " << *it << " clearing temp" << dendl;
		...
		if (it->is_pg(pgid, snap)) {
			if (snap != CEPH_NOSNAP) {
				dout(10)
							<< "load_pgs skipping snapped dir " << *it
									<< " (pg " << pgid << " snap " << snap
									<< ")" << dendl;
		...
		dout(10)
					<< "load_pgs ignoring unrecognized " << *it << dendl;
	}
	...
	for (map<spg_t, interval_set<snapid_t> >::iterator i = pgs.begin();
		...
		if (!head_pgs.count(pgid)) {
			dout(10)
						<< __func__ << ": " << pgid
								<< " has orphan snap collections " << i->second
								<< " with no head" << dendl;
			continue;
		}
		...
		if (!osdmap->have_pg_pool(pgid.pool())) {
			dout(10)
						<< __func__ << ": skipping PG " << pgid
								<< " because we don't have pool " << pgid.pool()
								<< dendl;
			continue;
		}
		...
		if (pgid.preferred() >= 0) {
			dout(10)
						<< __func__ << ": skipping localized PG " << pgid
								<< dendl;
		...
		dout(10)
					<< "pgid " << pgid << " coll " << coll_t(pgid) << dendl;
		bufferlist bl;
		epoch_t map_epoch = PG::peek_map_epoch(store, coll_t(pgid),
				service.infos_oid, &bl);

		PG *pg = _open_lock_pg(
				map_epoch == 0 ? osdmap : service.get_map(map_epoch), pgid);
		...
		if (pg->must_upgrade()) {
			...
			dout(10)
						<< "PG " << pg->info.pgid << " must upgrade..." << dendl;
		...
		pg->get_osdmap()->pg_to_up_acting_osds(pgid.pgid, &up, &up_primary,
				&acting, &primary);
		...
		dout(10)
					<< "load_pgs loaded " << *pg << " " << pg->pg_log.get_log()
							<< dendl;
		pg->unlock();
	}
	{
		RWLock::RLocker l(pg_map_lock);
		dout(0)
					<< "load_pgs opened " << pg_map.size() << " pgs" << dendl;
	}
...
void OSD::build_past_intervals_parallel() {
	...
	if (pis.empty()) {
		dout(10)
					<< __func__ << " nothing to build" << dendl;
		return;
	}

	dout(1)
				<< __func__ << " over " << cur_epoch << "-" << end_epoch
						<< dendl;
	...
	for (; cur_epoch <= end_epoch; cur_epoch++) {
		dout(10)
					<< __func__ << " epoch " << cur_epoch << dendl;
		last_map = cur_map;
		...
		for (map<PG*, pistate>::iterator i = pis.begin(); i != pis.end(); ++i) {
			...
			int up_primary;
			int primary;
			cur_map->pg_to_up_acting_osds(pg->info.pgid.pgid, &up, &up_primary,
					&acting, &primary);

			if (p.same_interval_since == 0) {
				dout(10)
							<< __func__ << " epoch " << cur_epoch << " pg "
									<< pg->info.pgid << " first map, acting "
									<< acting << " up " << up
			...
			bool new_interval = pg_interval_t::check_new_interval(p.primary,
					primary, p.old_acting, acting, p.up_primary, up_primary,
					p.old_up, up, p.same_interval_since,
					pg->info.history.last_epoch_clean, cur_map, last_map,
					pg->info.pgid.pgid, &pg->past_intervals, &debug);
			if (new_interval) {
				dout(10)
							<< __func__ << " epoch " << cur_epoch << " pg "
									<< pg->info.pgid << " " << debug.str()
									<< dendl;
				p.old_up = up;
				p.old_acting = acting;
				p.same_interval_since = cur_epoch;
 ...
 */
void OSD::handle_pg_peering_evt(spg_t pgid, const pg_info_t& info,
	...
	if (!_have_pg(pgid)) {
		...
		vector<int> up, acting;
		osdmap->pg_to_up_acting_osds(pgid.pgid, &up, &up_primary, &acting,
				&acting_primary);
		int role = osdmap->calc_pg_role(whoami, acting, acting.size());

		pg_history_t history = info.history;
		bool valid_history = project_pg_history(pgid, history, epoch, up,
				up_primary, acting, acting_primary);

		if (!valid_history || epoch < history.same_interval_since) {
			dout(10)
						<< "get_or_create_pg " << pgid << " acting changed in "
								<< history.same_interval_since << " (msg from "
								<< epoch << ")" << dendl;
			return;
		}
		...
		if (primary) {
			...
			if (info.dne()) {
				...
				if (creating_pgs.count(pgid)) {
					...
					dout(10)
								<< "get_or_create_pg " << pgid
										<< " DNE on source, but creation probe, ignoring"
										<< dendl;
					return;
				}
			}
		...
		spg_t resurrected;
		PGRef old_pg_state;
		res_result result = _try_resurrect_pg(service.get_osdmap(), pgid,
				&resurrected, &old_pg_state);
		...
		case RES_NONE: {
			...
			uint64_t expected_num_objects_pg = pp->expected_num_objects
					/ pg_num;
			::encode(pg_num, hint);
			...
			PG *pg = _create_lock_pg(get_map(epoch), pgid, create, false,
					result == RES_SELF, role, up, up_primary, acting,
					acting_primary, history, pi, *rctx.transaction);
		...
		case RES_SELF: {
			...
			old_pg_state->unlock();
			PG *pg = _create_lock_pg(old_osd_map, resurrected,
			false,
			false,
			true, old_role, old_up, old_up_primary, old_acting, old_primary,
					old_history, old_past_intervals, *rctx.transaction);
			pg->handle_create(&rctx);
			pg->write_if_dirty(*rctx.transaction);
			dispatch_context(rctx, pg, osdmap);

			dout(10)
						<< *pg << " is new (resurrected)" << dendl;
		...
		case RES_PARENT: {
			...
			pg_interval_map_t old_past_intervals = old_pg_state->past_intervals;
			old_pg_state->unlock();
			PG *parent = _create_lock_pg(old_osd_map, resurrected,
			false,
			false,
			true, old_role, old_up, old_up_primary, old_acting, old_primary,
					old_history, old_past_intervals, *rctx.transaction);
		...
		if (epoch < pg->info.history.same_interval_since) {
			dout(10)
						<< *pg << " get_or_create_pg acting changed in "
 ...
 */
void OSD::calc_priors_during(spg_t pgid, epoch_t start, epoch_t end,
		set<pg_shard_t>& pset) {
	dout(15)
				<< "calc_priors_during " << pgid << " [" << start << "," << end
						<< ")" << dendl;
...
bool OSD::project_pg_history(spg_t pgid, pg_history_t& h, epoch_t from,
		const vector<int>& currentup, int currentupprimary,
		const vector<int>& currentacting, int currentactingprimary) {
	dout(15)
				<< "project_pg_history " << pgid << " from " << from << " to "
						<< osdmap->get_epoch() << ", start " << h << dendl;

	epoch_t e;
	for (e = osdmap->get_epoch(); e > from; e--) {
		// verify during intermediate epoch (e-1)
		OSDMapRef oldmap = service.try_get_map(e - 1);
		if (!oldmap) {
			dout(15)
						<< __func__ << ": found map gap, returning false"
								<< dendl;
			return false;
		}
		...
		vector<int> up, acting;
		oldmap->pg_to_up_acting_osds(pgid.pgid, &up, &upprimary, &acting,
				&actingprimary);

		// acting set change?
		if ((actingprimary != currentactingprimary
				|| upprimary != currentupprimary || acting != currentacting
				|| up != currentup) && e > h.same_interval_since) {
			dout(15)
						<< "project_pg_history " << pgid
								<< " acting|up changed in " << e << " from "
								<< acting << "/" << up << " " << actingprimary
								<< "/" << upprimary << " -> " << currentacting
								<< "/" << currentup << " "
								<< currentactingprimary << "/"
								<< currentupprimary << dendl;
		...
		if ((up != currentup || upprimary != currentupprimary)
				&& e > h.same_up_since) {
			dout(15)
						<< "project_pg_history " << pgid << " up changed in "
								<< e << " from " << up << " " << upprimary
								<< " -> " << currentup << " "
								<< currentupprimary << dendl;
			h.same_up_since = e;
		...
		// primary change?
		if (OSDMap::primary_changed(actingprimary, acting, currentactingprimary,
				currentacting) && e > h.same_primary_since) {
			dout(15)
						<< "project_pg_history " << pgid
								<< " primary changed in " << e << dendl;
			h.same_primary_since = e;
		}

		if (h.same_interval_since >= e && h.same_up_since >= e
				&& h.same_primary_since >= e)
			break;
	}
	...
	dout(15)
				<< "project_pg_history end " << h << dendl;
	return true;
}
...
void OSD::_add_heartbeat_peer(int p) {
	...
	if (i == heartbeat_peers.end()) {
		pair<ConnectionRef, ConnectionRef> cons = service.get_con_osd_hb(p,
				osdmap->get_epoch());
		if (!cons.first)
			return;
		...
		if (cons.second) {
			...
			dout(10)
						<< "_add_heartbeat_peer: new peer osd." << p << " "
...
void OSD::_remove_heartbeat_peer(int n) {
	...
	dout(20)
				<< " removing heartbeat peer osd." << n << " "
						<< q->second.con_back->get_peer_addr() << " "
						<< (q->second.con_front ?
								q->second.con_front->get_peer_addr() :
								entity_addr_t()) << dendl;
...
void OSD::need_heartbeat_peer_update() {
	if (is_stopping())
		return;
	dout(20)
				<< "need_heartbeat_peer_update" << dendl;
...
void OSD::maybe_update_heartbeat_peers() {
	...
	if (is_waiting_for_healthy()) {
		...
		if (last_heartbeat_resample == utime_t()) {
			...
			if (dur > cct->_conf->osd_heartbeat_grace) {
				dout(10)
									<< "maybe_update_heartbeat_peers forcing update after "
									<< dur << " seconds" << dendl;
				heartbeat_set_peers_need_update();
				last_heartbeat_resample = now;
	...
	dout(10)
				<< "maybe_update_heartbeat_peers updating" << dendl;
	...
	for (set<int>::iterator p = want.begin(); p != want.end(); ++p) {
		dout(10)
					<< " adding neighbor peer osd." << *p << dendl;
		extras.insert(*p);
		_add_heartbeat_peer(*p);
	}
	...
	for (int n = start; n >= 0;) {
		...
		if (!extras.count(n) && !want.count(n) && n != whoami) {
			dout(10)
						<< " adding random peer osd." << n << dendl;
			extras.insert(n);
	...
	dout(10)
				<< "maybe_update_heartbeat_peers " << heartbeat_peers.size()
...
void OSD::handle_osd_ping(MOSDPing *m) {
	if (superblock.cluster_fsid != m->fsid) {
		dout(20)
					<< "handle_osd_ping from " << m->get_source_inst()
							<< " bad fsid " << m->fsid << " != "
	...
	case MOSDPing::PING: {
		...
		if (!cct->get_heartbeat_map()->is_healthy()) {
			dout(10)
								<< "internal heartbeat not healthy, dropping ping request"
								<< dendl;
			break;
		}

		Message *r = new MOSDPing(monc->get_fsid(), curmap->get_epoch(),
	...
	case MOSDPing::PING_REPLY: {
		...
		if (i != heartbeat_peers.end()) {
			if (m->get_connection() == i->second.con_back) {
				dout(25)
							<< "handle_osd_ping got reply from osd." << from
									<< " first_rx " << i->second.first_tx
									<< " last_tx " << i->second.last_tx
									<< " last_rx_back "
									<< i->second.last_rx_back << " -> "
									<< m->stamp << " last_rx_front "
									<< i->second.last_rx_front << dendl;
		...
		if (i->second.is_healthy(cutoff)) {
			...
			if (failure_queue.count(from)) {
				dout(10)
									<< "handle_osd_ping canceling queued failure report for osd."
									<< from << dendl;
				failure_queue.erase(from);
			}
			if (failure_pending.count(from)) {
				dout(10)
									<< "handle_osd_ping canceling in-flight failure report for osd."
									<< from << dendl;
	...
	case MOSDPing::YOU_DIED:
		dout(10)
					<< "handle_osd_ping " << m->get_source_inst()
...
void OSD::heartbeat_entry() {
	...
	while (!heartbeat_stop) {
		...
		dout(30)
					<< "heartbeat_entry sleeping for " << wait << dendl;
		heartbeat_cond.WaitInterval(cct, heartbeat_lock, w);
		if (is_stopping())
			return;
		dout(30)
					<< "heartbeat_entry woke up" << dendl;
	}
}
...
void OSD::heartbeat_check() {
	...
	if (age > (cct->_conf->osd_heartbeat_grace / 2)) {
		derr
					<< "skipping heartbeat_check, hbqueue max age: " << age
							<< dendl;
	...
	for (map<int, HeartbeatInfo>::iterator p = heartbeat_peers.begin();
		...
		dout(25)
					<< "heartbeat_check osd." << p->first << " first_tx "
							<< p->second.first_tx << " last_tx "
							<< p->second.last_tx << " last_rx_back "
							<< p->second.last_rx_back << " last_rx_front "
							<< p->second.last_rx_front << dendl;
		if (p->second.is_unhealthy(cutoff)) {
			if (p->second.last_rx_back == utime_t()
					|| p->second.last_rx_front == utime_t()) {
				derr
							<< "heartbeat_check: no reply from osd." << p->first
				...
				derr
							<< "heartbeat_check: no reply from osd." << p->first
									...
									<< " (cutoff " << cutoff << ")" << dendl;
							// fail
				failure_queue[p->first] = MIN(p->second.last_rx_back,
						p->second.last_rx_front);
			}
		}
	}
}
...
void OSD::heartbeat() {
	...
	dout(30)
				<< "heartbeat checking stats" << dendl;
	...
	dout(5)
				<< "heartbeat: " << service.get_osd_stat() << dendl;
	...
	for (map<int, HeartbeatInfo>::iterator i = heartbeat_peers.begin();
		...
		dout(30)
					<< "heartbeat sending ping to osd." << peer << dendl;
		i->second.con_back->send_message(
				new MOSDPing(monc->get_fsid(),
						service.get_osdmap()->get_epoch(), MOSDPing::PING,
						now));
...
bool OSD::heartbeat_reset(Connection *con) {
	...
	if (s) {
		...
		if (p != heartbeat_peers.end()
			...
			dout(10)
						<< "heartbeat_reset failed hb con " << con
								<< " for osd." << p->second.peer
								<< ", reopening" << dendl;
			...
			pair<ConnectionRef, ConnectionRef> newcon = service.get_con_osd_hb(
					p->second.peer, p->second.epoch);
			if (newcon.first) {
				...
				dout(10)
							<< "heartbeat_reset failed hb con " << con
									<< " for osd." << p->second.peer
									<< ", raced with osdmap update, closing out peer"
									<< dendl;
				heartbeat_peers.erase(p);
			}
		} else {
			dout(10)
						<< "heartbeat_reset closing (old) failed hb con " << con
								<< dendl;
		}
		heartbeat_lock.Unlock();
		s->put();
...
void OSD::tick() {
	...
	if (is_active() || is_waiting_for_healthy()) {
		...
		if (outstanding_pg_stats && timeout_mon_on_pg_stats
				&& (now - cct->_conf->osd_mon_ack_timeout)
						> last_pg_stats_ack) {
			dout(1)
		...
		if (now - last_pg_stats_sent
				> cct->_conf->osd_mon_report_interval_max) {
			osd_stat_updated = true;
			do_mon_report();
		} else if (now - last_mon_report
				> cct->_conf->osd_mon_report_interval_min) {
			do_mon_report();
		}
...
void OSD::check_ops_in_flight() {
	...
	if (op_tracker.check_ops_in_flight(warnings)) {
		for (vector<string>::iterator i = warnings.begin(); i != warnings.end();
				++i) {
			clog->warn() << *i;
		}
	}
...
// =========================================
bool remove_dir(CephContext *cct, ObjectStore *store, SnapMapper *mapper,
		OSDriver *osdriver, ObjectStore::Sequencer *osr, coll_t coll,
		DeletingStateRef dstate, ThreadPool::TPHandle &handle) {
	...
	while (!next.is_max()) {
		...
		store->collection_list_partial(coll, next, store->get_ideal_list_min(),
				store->get_ideal_list_max(), 0, &olist, &next);
		for (vector<ghobject_t>::iterator i = olist.begin(); i != olist.end();
				++i, ++num) {
...
void OSD::RemoveWQ::_process(pair<PGRef, DeletingStateRef> item,
	...
	for (list<coll_t>::iterator i = colls_to_remove.begin();
			i != colls_to_remove.end(); ++i) {
		bool cont = remove_dir(pg->cct, store, &mapper, &driver, pg->osr.get(),
				*i, item.second, handle);
		if (!cont)
			return;
	}
	...
	ObjectStore::Transaction *t = new ObjectStore::Transaction;
	PGLog::clear_info_log(pg->info.pgid, OSD::make_infos_oid(), pg->log_oid, t);
	...
	store->queue_transaction(pg->osr.get(), t, 0, // onapplied
			0, // oncommit
			0, // onreadable sync
			new ObjectStore::C_DeleteTransactionHolder<PGRef>(t, pg), // oncomplete
			TrackedOpRef());
...
void OSD::ms_handle_connect(Connection *con) {
	if (con->get_peer_type() == CEPH_ENTITY_TYPE_MON) {
		...
		if (is_stopping())
			return;
		dout(10)
					<< "ms_handle_connect on mon" << dendl;
		if (is_booting()) {
			start_boot();
		} else {
			send_alive();
			...
			monc->sub_want("osdmap", osdmap->get_epoch(),
...
void OSD::ms_handle_fast_connect(Connection *con) {
	if (con->get_peer_type() != CEPH_ENTITY_TYPE_MON) {
		...
		if (!s) {
			...
			s->con = con;
			dout(10)
						<< " new session (outgoing) " << s << " con=" << s->con
...
void OSD::ms_handle_fast_accept(Connection *con) {
	if (con->get_peer_type() != CEPH_ENTITY_TYPE_MON) {
		...
		if (!s) {
			...
			s->con = con;
			dout(10)
						<< "new session (incoming)" << s << " con=" << con
...
bool OSD::ms_handle_reset(Connection *con) {
	...
	dout(1)
				<< "ms_handle_reset con " << con << " session " << session
						<< dendl;
	if (!session)
		return false;
...
struct C_OSD_GetVersion: public Context {
	...
	C_OSD_GetVersion(OSD *o) :
			osd(o), oldest(0), newest(0) {
	}
	void finish(int r) {
		if (r >= 0)
...
void OSD::start_boot() {
	dout(10)
				<< "start_boot - have maps " << superblock.oldest_map << ".."
...
void OSD::_maybe_boot(epoch_t oldest, epoch_t newest) {
	...
	if (is_stopping())
		return;
	dout(10)
				<< "_maybe_boot mon has osdmaps " << oldest << ".." << newest
						<< dendl;

	if (is_initializing()) {
		dout(10)
					<< "still initializing" << dendl;
		return;
	}
	...
	if (osdmap->test_flag(CEPH_OSDMAP_NOUP)) {
		dout(5)
					<< "osdmap NOUP flag is set, waiting for it to clear"
							<< dendl;
	} else if (is_waiting_for_healthy() || !_is_healthy()) {
		// if we are not healthy, do not mark ourselves up (yet)
		dout(1)
					<< "not healthy; waiting to boot" << dendl;
		if (!is_waiting_for_healthy())
...
bool OSD::_is_healthy() {
	if (!cct->get_heartbeat_map()->is_healthy()) {
		dout(1)
					<< "is_healthy false -- internal heartbeat failed" << dendl;
		return false;
	}
	...
	if (is_waiting_for_healthy()) {
		...
		if ((float) up
				< (float) num * cct->_conf->osd_heartbeat_min_healthy_ratio) {
			dout(1)
						<< "is_healthy false -- only " << up << "/" << num
...
void OSD::_send_boot() {
	...
	if (cluster_addr.is_blank_ip()) {
		...
		dout(10)
					<< " assuming cluster_addr ip matches client_addr" << dendl;
	} else {
	...
	if (hb_back_addr.is_blank_ip()) {
		...
		dout(10)
					<< " assuming hb_back_addr ip matches cluster_addr" << dendl;
	} else {
		Session *s = static_cast<Session*>(local_connection->get_priv());
		if (s)
			s->put();
		else
			hb_back_server_messenger->ms_deliver_handle_fast_connect(
					local_connection);
	}
	...
	if (hb_front_addr.is_blank_ip()) {
		...
		dout(10)
					<< " assuming hb_front_addr ip matches client_addr" << dendl;
	} else {
		Session *s = static_cast<Session*>(local_connection->get_priv());
		if (s)
			s->put();
		else
			hb_front_server_messenger->ms_deliver_handle_fast_connect(
					local_connection);
	}
	...
	dout(10)
				<< " client_addr " << client_messenger->get_myaddr()
						...
						<< ", hb_front_addr " << hb_front_addr << dendl;
	_collect_metadata(&mboot->metadata);
	monc->send_mon_message(mboot);
}

bool OSD::_lsb_release_set(char *buf, const char *str, map<string, string> *pm,
		const char *key) {
...
void OSD::_lsb_release_parse(map<string, string> *pm) {
	...
	if (!fp) {
		int ret = -errno;
		derr
							<< "lsb_release_parse - failed to call lsb_release binary with error: "
	...
	while (fgets(buf, sizeof(buf) - 1, fp) != NULL) {
		...
		derr
					<< "unhandled output: " << buf << dendl;
	}

	if (pclose(fp)) {
		int ret = -errno;
		derr
					<< "lsb_release_parse - pclose failed: "
							<< cpp_strerror(ret) << dendl;
	}
}
...
void OSD::queue_want_up_thru(epoch_t want) {
	...
	if (want > up_thru_wanted) {
		dout(10)
					<< "queue_want_up_thru now " << want << " (was "
							<< up_thru_wanted << ")" << ", currently " << cur
							<< dendl;
		up_thru_wanted = want;
		...
		dout(10)
					<< "queue_want_up_thru want " << want << " <= queued "
...
void OSD::send_alive() {
	...
	dout(10)
				<< "send_alive up_thru currently " << up_thru << " want "
						<< up_thru_wanted << dendl;
	if (up_thru_wanted > up_thru) {
		up_thru_pending = up_thru_wanted;
		dout(10)
					<< "send_alive want " << up_thru_wanted << dendl;
		monc->send_mon_message(
				new MOSDAlive(osdmap->get_epoch(), up_thru_wanted));
	}
}
...
void OSD::send_failures() {
	...
	while (!failure_queue.empty()) {
		...
		monc->send_mon_message(
				new MOSDFailure(monc->get_fsid(), i, failed_for,
						osdmap->get_epoch()));
...
void OSD::send_pg_stats(const utime_t &now) {
	...
	if (osd_stat_updated || !pg_stat_queue.empty()) {
		...
		dout(10)
					<< "send_pg_stats - " << pg_stat_queue.size()
							<< " pgs updated" << dendl;

		utime_t had_for(now);
		had_for -= had_map_since;

		MPGStats *m = new MPGStats(monc->get_fsid(), osdmap->get_epoch(),
				had_for);
		...
		while (!p.end()) {
			...
			if (pg->pg_stats_publish_valid) {
				...
				dout(25)
							<< " sending " << pg->info.pgid << " "
									<< pg->pg_stats_publish.reported_epoch
									<< ":" << pg->pg_stats_publish.reported_seq
									<< dendl;
			} else {
				dout(25)
							<< " NOT sending " << pg->info.pgid << " "
									<< pg->pg_stats_publish.reported_epoch
									<< ":" << pg->pg_stats_publish.reported_seq
									<< ", not valid" << dendl;
			}
...
void OSD::flush_pg_stats() {
	...
	dout(10)
				<< "flush_pg_stats waiting for stats tid " << tid << " to flush"
						<< dendl;
	while (tid > pg_stat_tid_flushed)
		pg_stat_queue_cond.Wait(pg_stat_queue_lock);
	dout(10)
				<< "flush_pg_stats finished waiting for stats tid " << tid
						<< " to flush" << dendl;
	pg_stat_queue_lock.Unlock();
...
// tell <osd.n> commands.  Validation of osd.n must be special-cased in client
				...
				COMMAND("injectargs "
						...
						COMMAND(
								...
								"log a message to the cluster log", "osd", "rw",
								"cli,rest")
				COMMAND("bench "
						...
						COMMAND("debug_dump_missing "
								...
								"dump missing objects to a named file", "osd",
								"r", "cli,rest")
						COMMAND("debug kick_recovery_wq "
								"name=delay,type=CephInt,range=0",
								"set osd_recovery_delay_start to <val>", "osd",
								"rw", "cli,rest")
				...
				COMMAND("dump_pg_recovery_stats", "dump pg recovery statistics",
						"osd", "r", "cli,rest")
						COMMAND("reset_pg_recovery_stats",
								"reset pg recovery statistics", "osd", "rw",
								"cli,rest") };

void OSD::do_command(Connection *con, ceph_tid_t tid, vector<string>& cmd,
		bufferlist& data) {
	int r = 0;
	stringstream ss, ds;
	string rs;
	...
	if (prefix == "get_command_descriptions") {
		...
		for (OSDCommand *cp = osd_commands;
			...
			dump_cmddesc_to_json(f, secname.str(), cp->cmdstring,
					cp->helpstring, cp->module, cp->perm, cp->availability);
			cmdnum++;
		}
	...
	if (prefix == "version") {
		...
		for (vector<string>::iterator a = ++argsvec.begin(); a != argsvec.end();
				++a)
			args += " " + *a;
		osd_lock.Unlock();
	...
	// 'tell <pgid>' (which comes in without any of that prefix)?
			...
			|| (cmd_getval(cct, cmdmap, "pgid", pgidstr)
					&& (prefix == "query" || prefix == "mark_unfound_lost"
							|| prefix == "list_missing"))) {
		pg_t pgid;
		...
		if (!cmd_getval(cct, cmdmap, "pgid", pgidstr)) {
			...
			spg_t pcand;
			if (osdmap->get_primary_shard(pgid, &pcand) && _have_pg(pcand)) {
				...
				if (pg->is_primary()) {
					...
					// has changed.
					service.send_incremental_map(osdmap->get_epoch() - 1, con,
							osdmap);
		...
		if (bsize > (int64_t) g_conf->osd_bench_max_block_size) {
			...
			ss << "block 'size' values are capped at "
					<< prettybyte_t(g_conf->osd_bench_max_block_size)
					<< ". If you wish to use"
			...
			int64_t max_count = bsize * duration
					* g_conf->osd_bench_small_size_max_iops;
			if (count > max_count) {
				ss << "'count' values greater than " << max_count
						<< " for a block size of " << prettybyte_t(bsize)
						<< ", assuming "
			...
			int64_t max_count = g_conf->osd_bench_large_size_max_throughput
					* duration;
			if (count > max_count) {
				ss << "'count' values greater than " << max_count
						<< " for a block size of " << prettybyte_t(bsize)
						<< ", assuming "
						<< prettybyte_t(
								g_conf->osd_bench_large_size_max_throughput)
		...
		dout(1)
					<< " bench count " << count << " bsize "
							<< prettybyte_t(bsize) << dendl;
		...
		if (f) {
			...
			ss << "bench: wrote " << prettybyte_t(count) << " in blocks of "
					<< prettybyte_t(bsize) << " in " << (end - start)
		...
		std::set<spg_t> keys;
		RWLock::RLocker l(pg_map_lock);
		for (ceph::unordered_map<spg_t, PG*>::const_iterator pg_map_e =
				pg_map.begin(); pg_map_e != pg_map.end(); ++pg_map_e) {
			keys.insert(pg_map_e->first);
		}

		fout << "*** osd " << whoami << ": dump_missing ***" << std::endl;
		for (std::set<spg_t>::iterator p = keys.begin(); p != keys.end(); ++p) {
			...
			for (; mi != mend; ++mi) {
				...
				if (pg->missing_loc.is_unfound(mi->first))
					fout << " unfound ";
				const set<pg_shard_t> &mls(
						pg->missing_loc.get_locations(mi->first));
				if (mls.empty())
					continue;
...
bool OSD::heartbeat_dispatch(Message *m) {
	dout(30)
				<< "heartbeat_dispatch " << m << dendl;
	switch (m->get_type()) {

	case CEPH_MSG_PING:
		dout(10)
					<< "ping from " << m->get_source_inst() << dendl;
		m->put();
		break;
	...
	default:
		dout(0)
					<< "dropping unexpected message " << *m << " from "
...
bool OSD::ms_dispatch(Message *m) {
	...
	while (dispatch_running) {
		dout(10)
							<< "ms_dispatch waiting for other dispatch thread to complete"
							<< dendl;
...
void OSD::update_waiting_for_pg(Session *session, OSDMapRef newmap) {
	...
	for (map<spg_t, list<OpRequestRef> >::iterator i = from.begin();
		...
		if (!newmap->have_pg_pool(i->first.pool())) {
			...
			if (i->first.is_split(session->osdmap->get_pg_num(i->first.pool()),
					newmap->get_pg_num(i->first.pool()), &children)) {
				for (set<spg_t>::iterator child = children.begin();
					...
					OSD::split_list(&i->second, &child_ops, child->ps(),
							split_bits);
					if (!child_ops.empty()) {
...
void OSD::session_notify_pg_create(Session *session, OSDMapRef osdmap,
		spg_t pgid) {
	assert(session->session_dispatch_lock.is_locked());
	update_waiting_for_pg(session, osdmap);
	map<spg_t, list<OpRequestRef> >::iterator i = session->waiting_for_pg.find(
			pgid);
	if (i != session->waiting_for_pg.end()) {
		session->waiting_on_map.splice(session->waiting_on_map.begin(),
				i->second);
...
void OSD::session_notify_pg_cleared(Session *session, OSDMapRef osdmap,
		spg_t pgid) {
	assert(session->session_dispatch_lock.is_locked());
	update_waiting_for_pg(session, osdmap);
...
bool OSD::ms_verify_authorizer(Connection *con, int peer_type, int protocol,
	...
	case CEPH_ENTITY_TYPE_OSD:
		authorize_handler = authorize_handler_cluster_registry->get_handler(
				protocol);
		break;
	default:
		authorize_handler = authorize_handler_service_registry->get_handler(
				protocol);
	}
	if (!authorize_handler) {
		dout(0)
					<< "No AuthAuthorizeHandler found for protocol " << protocol
							<< dendl;
		isvalid = false;
		return true;
	}
...
epoch_t op_required_epoch(OpRequestRef op) {
	...
	case MSG_OSD_SUBOPREPLY:
		return replica_op_required_epoch<MOSDSubOpReply, MSG_OSD_SUBOPREPLY>(op);
	case MSG_OSD_PG_PUSH:
		return replica_op_required_epoch<MOSDPGPush, MSG_OSD_PG_PUSH>(op);
	case MSG_OSD_PG_PULL:
		return replica_op_required_epoch<MOSDPGPull, MSG_OSD_PG_PULL>(op);
	...
	case MSG_OSD_EC_WRITE_REPLY:
		return replica_op_required_epoch<MOSDECSubOpWriteReply,
				MSG_OSD_EC_WRITE_REPLY>(op);
	case MSG_OSD_EC_READ:
		return replica_op_required_epoch<MOSDECSubOpRead, MSG_OSD_EC_READ>(op);
	case MSG_OSD_EC_READ_REPLY:
		return replica_op_required_epoch<MOSDECSubOpReadReply,
				MSG_OSD_EC_READ_REPLY>(op);
	default:
		assert(0);
		return 0;
	}
...
void OSD::handle_rep_scrub(MOSDRepScrub *m) {
	dout(10)
				<< "queueing MOSDRepScrub " << *m << dendl;
	rep_scrub_wq.queue(m);
}

void OSD::handle_scrub(MOSDScrub *m) {
	dout(10)
				<< "handle_scrub " << *m << dendl;
	if (!require_mon_peer(m))
		return;
	if (m->fsid != monc->get_fsid()) {
		dout(0)
					<< "handle_scrub fsid " << m->fsid << " != "
							<< monc->get_fsid() << dendl;
		m->put();
		return;
	}
...
bool OSD::scrub_should_schedule() {
	...
	if (getloadavg(loadavgs, 1) != 1) {
		dout(10)
					<< "scrub_should_schedule couldn't read loadavgs\n" << dendl;
		return false;
	}

	if (loadavgs[0] >= cct->_conf->osd_scrub_load_threshold) {
		dout(20)
					<< "scrub_should_schedule loadavg " << loadavgs[0]
							<< " >= max "
	...
	dout(20)
				<< "scrub_should_schedule loadavg " << loadavgs[0] << " < max "
						<< cct->_conf->osd_scrub_load_threshold << " = yes"
						<< dendl;
...
void OSD::sched_scrub() {
	...
	dout(20)
				<< "sched_scrub load_is_low=" << (int) load_is_low << dendl;
	...
	if (service.first_scrub_stamp(&pos)) {
		do {
			...
			dout(30)
						<< "sched_scrub examine " << pgid << " at " << t
								<< dendl;

			utime_t diff = now - t;
			if ((double) diff < cct->_conf->osd_scrub_min_interval) {
				dout(10)
							<< "sched_scrub " << pgid << " at " << t << ": "
									<< (double) diff << " < min ("
									<< cct->_conf->osd_scrub_min_interval
									<< " seconds)" << dendl;
				break;
			}
			if ((double) diff < cct->_conf->osd_scrub_max_interval
					&& !load_is_low) {
				// save ourselves some effort
				dout(10)
							<< "sched_scrub " << pgid << " high load at " << t
									<< ": " << (double) diff << " < max ("
...
struct C_OnMapApply: public Context {
	...
	epoch_t e;
	C_OnMapApply(OSDService *service, ObjectStore::Transaction *t,
			const list<OSDMapRef> &pinned_maps, epoch_t e) :
...
void OSD::handle_osd_map(MOSDMap *m) {
	...
	if (is_initializing()) {
		dout(0)
					<< "ignoring osdmap until we have initialized" << dendl;
		m->put();
		return;
	}
	...
	if (session
		...
		//not enough perms!
		dout(10)
					<< "got osd map from Session " << session
							<< " which we can't take maps from (not a mon or osd)"
							<< dendl;
		m->put();
		session->put();
		return;
	...
	dout(3)
				<< "handle_osd_map epochs [" << first << "," << last
						<< "], i have " << osdmap->get_epoch() << ", src has ["
						<< m->oldest_map << "," << m->newest_map << "]" << dendl;
	...
	if (first > osdmap->get_epoch() + 1) {
		dout(10)
					<< "handle_osd_map message skips epochs "
							<< osdmap->get_epoch() + 1 << ".." << (first - 1)
							<< dendl;
	...
	for (epoch_t e = start; e <= last; e++) {
		...
		if (p != m->maps.end()) {
			dout(10)
						<< "handle_osd_map  got full map for epoch " << e
								<< dendl;
			OSDMap *o = new OSDMap;
		...
		if (p != m->incremental_maps.end()) {
			dout(10)
						<< "handle_osd_map  got inc map for epoch " << e
								<< dendl;
			bufferlist& bl = p->second;
			...
			if (o->apply_incremental(inc) < 0) {
				derr
							<< "ERROR: bad fsid?  i have " << osdmap->get_fsid()
									<< " and inc has " << inc.fsid << dendl;assert(
						0 == "bad fsid");
			}
	...
	if (superblock.oldest_map) {
		int num = 0;
		epoch_t min(
				MIN(m->oldest_map, service.map_cache.cached_key_lower_bound()));
		for (epoch_t e = superblock.oldest_map; e < min; ++e) {
			dout(20)
						<< " removing old osdmap epoch " << e << dendl;
	...
	for (epoch_t cur = start; cur <= superblock.newest_map; cur++) {
		...
		for (set<int>::iterator p = old.begin(); p != old.end(); ++p) {
			if (*p != whoami && osdmap->have_inst(*p) &&          // in old map
		...
		if (!osdmap->exists(whoami)) {
			...
			if (!osdmap->is_up(whoami)) {
				...
				clog->error() << "map e" << osdmap->get_epoch()
						...
						<< osdmap->get_hb_front_addr(whoami) << " != my "
						<< hb_front_server_messenger->get_myaddr() << ")";
			...
			if (!service.is_stopping()) {
				...
				avoid_ports.insert(
						hb_back_server_messenger->get_myaddr().get_port());
				avoid_ports.insert(
						hb_front_server_messenger->get_myaddr().get_port());
	...
	write_superblock(t);
	store->queue_transaction(0, _t,
			new C_OnMapApply(&service, _t, pinned_maps, osdmap->get_epoch()), 0,
			fin);
	service.publish_superblock(superblock);
	...
	if (!is_active()) {
		dout(10)
					<< " not yet active; waiting for peering wq to drain"
							<< dendl;
		peering_wq.drain();
	} else {
		activate_map();
	...
	if (m->newest_map && m->newest_map > last) {
		dout(10)
					<< " msg say newest map is " << m->newest_map
...
void OSD::check_osdmap_features(ObjectStore *fs) {
...

	{
		...
		uint64_t mask;
		uint64_t features = osdmap->get_features(entity_name_t::TYPE_CLIENT,
				&mask);
		if ((p.features_required & mask) != features) {
			dout(0)
						<< "crush map has features " << features
	...
	{
		Messenger::Policy p = client_messenger->get_policy(
				entity_name_t::TYPE_MON);
		uint64_t mask;
		uint64_t features = osdmap->get_features(entity_name_t::TYPE_MON,
				&mask);
		if ((p.features_required & mask) != features) {
			dout(0)
						<< "crush map has features " << features << " was "
								<< p.features_required
	...
	{
		Messenger::Policy p = cluster_messenger->get_policy(
				entity_name_t::TYPE_OSD);
		uint64_t mask;
		uint64_t features = osdmap->get_features(entity_name_t::TYPE_OSD,
				&mask);
...
bool OSD::advance_pg(epoch_t osd_epoch, PG *pg, ThreadPool::TPHandle &handle,
		PG::RecoveryCtx *rctx, set<boost::intrusive_ptr<PG> > *new_pgs) {
	assert(pg->is_locked());
	...
	for (; next_epoch <= osd_epoch && next_epoch <= max; ++next_epoch) {
		...
		if (!nextmap) {
			dout(20)
						<< __func__ << " missing map " << next_epoch << dendl;
		...
		int up_primary, acting_primary;
		nextmap->pg_to_up_acting_osds(pg->info.pgid.pgid, &newup, &up_primary,
		...
		if (parent.is_split(lastmap->get_pg_num(pg->pool.id),
				nextmap->get_pg_num(pg->pool.id), &children)) {
...
void OSD::advance_map(ObjectStore::Transaction& t, C_Contexts *tfin) {
	...
	dout(7)
				<< "advance_map epoch " << osdmap->get_epoch() << dendl;
	...
	if (!up_epoch && osdmap->is_up(whoami)
		...
		dout(10)
					<< "up_epoch is " << up_epoch << dendl;
		if (!boot_epoch) {
			boot_epoch = osdmap->get_epoch();
			dout(10)
						<< "boot_epoch is " << boot_epoch << dendl;
		}
		service.set_epochs(&boot_epoch, &up_epoch, NULL);
	}

	// scan pg creations
	ceph::unordered_map<spg_t, create_pg_info>::iterator n =
			creating_pgs.begin();
	while (n != creating_pgs.end()) {
		...
		if (primary != whoami) {
			dout(10)
						<< " no longer primary for " << pgid
								<< ", stopping creation" << dendl;
...
void OSD::consume_map() {
	assert(osd_lock.is_locked());
	dout(7)
				<< "consume_map version " << osdmap->get_epoch() << dendl;
	...
	// scan pg's
	{
		...
		for (ceph::unordered_map<spg_t, PG*>::iterator it = pg_map.begin();
			...
			if (!osdmap->have_pg_pool(pg->info.pgid.pool())) {
				...
				service.init_splits_between(it->first, service.get_osdmap(),
						osdmap);
			}
	...
	for (list<PGRef>::iterator i = to_remove.begin(); i != to_remove.end();
			to_remove.erase(i++)) {
	...
	for (set<spg_t>::iterator p = pgs_to_check.begin(); p != pgs_to_check.end();
			++p) {
		vector<int> acting;
...
void OSD::activate_map() {
	...
	dout(7)
				<< "activate_map version " << osdmap->get_epoch() << dendl;

	if (osdmap->test_flag(CEPH_OSDMAP_FULL)) {
		dout(10)
					<< " osdmap flagged full, doing onetime osdmap subscribe"
							<< dendl;
		osdmap_subscribe(osdmap->get_epoch() + 1, true);
	}
	...
	if (osdmap->test_flag(CEPH_OSDMAP_NORECOVER)) {
		if (!paused_recovery) {
			dout(1)
						<< "pausing recovery (NORECOVER flag set)" << dendl;
		...
		if (paused_recovery) {
			dout(1)
						<< "resuming recovery (NORECOVER flag cleared)" << dendl;
...
bool OSD::require_mon_peer(Message *m) {
	if (!m->get_connection()->peer_is_mon()) {
		dout(0)
					<< "require_mon_peer received from non-mon "
							<< m->get_connection()->get_peer_addr() << " " << *m
							<< dendl;
		m->put();
		return false;
	}
...
bool OSD::require_osd_peer(OpRequestRef& op) {
	if (!op->get_req()->get_connection()->peer_is_osd()) {
		dout(0)
					<< "require_osd_peer received from non-osd "
...
bool OSD::require_self_aliveness(OpRequestRef& op, epoch_t epoch) {
	...
	if (epoch < up_epoch) {
		dout(7)
					<< "from pre-up epoch " << epoch << " < " << up_epoch
							<< dendl;
		return false;
	}
	...
	if (!is_active()) {
		dout(7)
					<< "still in boot state, dropping message "
							<< *op->get_req() << dendl;
		return false;
	}
...
bool is_fast_dispatch) {
	...
	if (!map->have_inst(from)
		...
		dout(5)
					<< "from dead osd." << from << ", marking down, "
							<< " msg was " << m->get_source_inst().addr
							<< " expected "
							<< (map->have_inst(from) ?
									map->get_cluster_addr(from) :
									entity_addr_t()) << dendl;
		ConnectionRef con = m->get_connection();
		con->mark_down();
		Session *s = static_cast<Session*>(con->get_priv());
...
bool is_fast_dispatch) {
	Message *m = op->get_req();
	dout(15)
				<< "require_same_or_newer_map " << epoch << " (i am "
	...
	if (epoch > osdmap->get_epoch()) {
		dout(7)
					<< "waiting for newer map epoch " << epoch << " > my "
...
bool OSD::can_create_pg(spg_t pgid) {
	...
	if (!creating_pgs[pgid].prior.empty()) {
		dout(10)
					<< "can_create_pg " << pgid << " - waiting for priors "
							<< creating_pgs[pgid].prior << dendl;
		return false;
	}

	dout(10)
				<< "can_create_pg " << pgid << " - can create now" << dendl;
	return true;
}
...
void OSD::split_pgs(PG *parent, const set<spg_t> &childpgids,
	...
	unsigned pg_num = nextmap->get_pg_num(parent->pool.id);
	parent->update_snap_mapper_bits(parent->info.pgid.get_split_bits(pg_num));
	...
	for (set<spg_t>::const_iterator i = childpgids.begin();
			i != childpgids.end(); ++i, ++stat_iter) {
		assert(stat_iter != updated_stats.end());
		dout(10)
					<< "Splitting " << *parent << " into " << *i << dendl;
		...
		dout(10)
					<< "pg_num is " << pg_num << dendl;
		dout(10)
					<< "m_seed " << i->ps() << dendl;
		dout(10)
					<< "split_bits is " << split_bits << dendl;

		parent->split_colls(*i, split_bits, i->ps(), rctx->transaction);
...
void OSD::handle_pg_create(OpRequestRef op) {
	...
	if (debug_drop_pg_create_left >= 0) {
		...
		if (debug_drop_pg_create_left >= 0) {
			dout(0)
								<< "DEBUG dropping/ignoring pg_create, will drop the next "
	...
	for (map<pg_t, pg_create_t>::iterator p = m->mkpg.begin();
		...
		if (on.preferred() >= 0) {
			dout(20)
						<< "ignoring localized pg " << on << dendl;
			continue;
		}

		if (!osdmap->have_pg_pool(on.pool())) {
			dout(20)
						<< "ignoring pg on deleted pool " << on << dendl;
			continue;
		}

		dout(20)
					<< "mkpg " << on << " e" << created << "@" << ci->second
							<< dendl;
		...
		if (up_primary != whoami) {
			dout(10)
						<< "mkpg " << on << "  not primary (role=" << role
								<< "), skipping" << dendl;
			continue;
		}
		if (up != acting) {
			dout(10)
						<< "mkpg " << on << "  up " << up << " != acting "
								<< acting << ", ignoring" << dendl;
		...
		if (_have_pg(pgid)) {
			dout(10)
						<< "mkpg " << pgid << "  already exists, skipping"
								<< dendl;
			continue;
		}
		...
		bool valid_history = project_pg_history(pgid, history, created, up,
				up_primary, acting, acting_primary);
		...
		for (set<pg_shard_t>::iterator p = pset.begin(); p != pset.end(); ++p)
			if (osdmap->is_up(p->osd))
				(*rctx.query_map)[p->osd][spg_t(pgid.pgid, p->shard)] =
						pg_query_t(pg_query_t::INFO, p->shard, pgid.shard,
								history, osdmap->get_epoch());
		...
		if (can_create_pg(pgid)) {
			...
			pg = _create_lock_pg(osdmap, pgid, true, false, false, 0,
					creating_pgs[pgid].acting, whoami,
					creating_pgs[pgid].acting, whoami, history, pi,
					*rctx.transaction);
...
PG::RecoveryCtx OSD::create_context() {
	...
	map<int, map<spg_t, pg_query_t> > *query_map = new map<int,
			map<spg_t, pg_query_t> >;
	map<int, vector<pair<pg_notify_t, pg_interval_map_t> > > *notify_list =
			new map<int, vector<pair<pg_notify_t, pg_interval_map_t> > >;
	map<int, vector<pair<pg_notify_t, pg_interval_map_t> > > *info_map =
			new map<int, vector<pair<pg_notify_t, pg_interval_map_t> > >;
	PG::RecoveryCtx rctx(query_map, info_map, notify_list, on_applied, on_safe,
			t);
	return rctx;
}
...
void OSD::dispatch_context_transaction(PG::RecoveryCtx &ctx, PG *pg,
	...
	if (!ctx.transaction->empty()) {
		ctx.on_applied->add(
				new ObjectStore::C_DeleteTransaction(ctx.transaction));
		int tr = store->queue_transaction(pg->osr.get(), ctx.transaction,
				ctx.on_applied, ctx.on_safe, NULL, TrackedOpRef(), handle);
		assert(tr == 0);
	...
	if (!pg->actingbackfill.empty()) {
		...
		for (unsigned i = 0; i < pg->acting.size(); ++i) {
			if (pg->acting[i] == CRUSH_ITEM_NONE)
				continue;
			tmpacting.insert(
					pg_shard_t(pg->acting[i],
							pg->pool.info.ec_pool() ?
									shard_id_t(i) : shard_id_t::NO_SHARD));
		}
	}

	for (set<pg_shard_t>::iterator i = tmpacting.begin(); i != tmpacting.end();
			++i) {
		if (i->osd == whoami || i->osd == CRUSH_ITEM_NONE)
			continue;
		ConnectionRef conn = service.get_con_osd_cluster(i->osd,
...
void OSD::do_notifies(
		map<int, vector<pair<pg_notify_t, pg_interval_map_t> > >& notify_list,
		OSDMapRef curmap) {
	for (map<int, vector<pair<pg_notify_t, pg_interval_map_t> > >::iterator it =
			notify_list.begin(); it != notify_list.end(); ++it) {
		if (!curmap->is_up(it->first)) {
			dout(20)
						<< __func__ << " skipping down osd." << it->first
								<< dendl;
			continue;
		}
		ConnectionRef con = service.get_con_osd_cluster(it->first,
				curmap->get_epoch());
		if (!con) {
			dout(20)
						<< __func__ << " skipping osd." << it->first
								<< " (NULL con)" << dendl;
			continue;
		}
		...
		if (con->has_feature(CEPH_FEATURE_INDEP_PG_MAP)) {
			dout(7)
						<< __func__ << " osd " << it->first << " on "
								<< it->second.size() << " PGs" << dendl;
			MOSDPGNotify *m = new MOSDPGNotify(curmap->get_epoch(), it->second);
			con->send_message(m);
		} else {
			dout(7)
						<< __func__ << " osd " << it->first
			...
			for (vector<pair<pg_notify_t, pg_interval_map_t> >::iterator i =
				...
				list[0] = *i;
				MOSDPGNotify *m = new MOSDPGNotify(i->first.epoch_sent, list);
				con->send_message(m);
			}
		}
	}
...
void OSD::do_queries(map<int, map<spg_t, pg_query_t> >& query_map,
	...
	for (map<int, map<spg_t, pg_query_t> >::iterator pit = query_map.begin();
			pit != query_map.end(); ++pit) {
		if (!curmap->is_up(pit->first)) {
			dout(20)
						<< __func__ << " skipping down osd." << pit->first
								<< dendl;
			continue;
		}
		int who = pit->first;
		ConnectionRef con = service.get_con_osd_cluster(who,
				curmap->get_epoch());
		if (!con) {
			dout(20)
						<< __func__ << " skipping osd." << who << " (NULL con)"
								<< dendl;
			continue;
		}
		...
		if (con->has_feature(CEPH_FEATURE_INDEP_PG_MAP)) {
			dout(7)
						<< __func__ << " querying osd." << who << " on "
								<< pit->second.size() << " PGs" << dendl;
			MOSDPGQuery *m = new MOSDPGQuery(curmap->get_epoch(), pit->second);
			con->send_message(m);
		} else {
			dout(7)
						<< __func__ << " querying osd." << who
								<< " sending seperate messages on "
								<< pit->second.size() << " PGs" << dendl;
...
void OSD::do_infos(
		map<int, vector<pair<pg_notify_t, pg_interval_map_t> > >& info_map,
		OSDMapRef curmap) {
	for (map<int, vector<pair<pg_notify_t, pg_interval_map_t> > >::iterator p =
			info_map.begin(); p != info_map.end(); ++p) {
		if (!curmap->is_up(p->first)) {
			dout(20)
						<< __func__ << " skipping down osd." << p->first
								<< dendl;
			continue;
		}
		for (vector<pair<pg_notify_t, pg_interval_map_t> >::iterator i =
				p->second.begin(); i != p->second.end(); ++i) {
			dout(20)
						<< __func__ << " sending info " << i->first.info
								<< " to shard " << p->first << dendl;
		}
		ConnectionRef con = service.get_con_osd_cluster(p->first,
				curmap->get_epoch());
		if (!con) {
			dout(20)
						<< __func__ << " skipping osd." << p->first
								<< " (NULL con)" << dendl;
			continue;
		}
...
void OSD::handle_pg_notify(OpRequestRef op) {
	...
	dout(7)
				<< "handle_pg_notify from " << m->get_source() << dendl;
	int from = m->get_source().num();
	...
	for (vector<pair<pg_notify_t, pg_interval_map_t> >::iterator it =
			m->get_pg_list().begin(); it != m->get_pg_list().end(); ++it) {

		if (it->first.info.pgid.preferred() >= 0) {
			dout(20)
						<< "ignoring localized pg " << it->first.info.pgid
								<< dendl;
			continue;
		}

		handle_pg_peering_evt(spg_t(it->first.info.pgid.pgid, it->first.to),
				it->first.info, it->second, it->first.query_epoch,
				pg_shard_t(from, it->first.from), true,
				PG::CephPeeringEvtRef(
						new PG::CephPeeringEvt(it->first.epoch_sent,
								it->first.query_epoch,
								PG::MNotifyRec(pg_shard_t(from, it->first.from),
										it->first,
...
void OSD::handle_pg_log(OpRequestRef op) {
	...
	dout(7)
				<< "handle_pg_log " << *m << " from " << m->get_source()
						<< dendl;
	...
	if (m->info.pgid.preferred() >= 0) {
		dout(10)
					<< "ignoring localized pg " << m->info.pgid << dendl;
		return;
	}

	op->mark_started();
	handle_pg_peering_evt(spg_t(m->info.pgid.pgid, m->to), m->info,
			m->past_intervals, m->get_epoch(), pg_shard_t(from, m->from), false,
			PG::CephPeeringEvtRef(
					new PG::CephPeeringEvt(m->get_epoch(), m->get_query_epoch(),
...
void OSD::handle_pg_info(OpRequestRef op) {
	...
	dout(7)
				<< "handle_pg_info " << *m << " from " << m->get_source()
						<< dendl;
	...
	for (vector<pair<pg_notify_t, pg_interval_map_t> >::iterator p =
			m->pg_list.begin(); p != m->pg_list.end(); ++p) {
		if (p->first.info.pgid.preferred() >= 0) {
			dout(10)
						<< "ignoring localized pg " << p->first.info.pgid
								<< dendl;
			continue;
		}

		handle_pg_peering_evt(spg_t(p->first.info.pgid.pgid, p->first.to),
				p->first.info, p->second, p->first.epoch_sent,
				pg_shard_t(from, p->first.from), false,
				PG::CephPeeringEvtRef(
						new PG::CephPeeringEvt(p->first.epoch_sent,
								p->first.query_epoch,
								PG::MInfoRec(pg_shard_t(from, p->first.from),
...
void OSD::handle_pg_trim(OpRequestRef op) {
	...
	dout(7)
				<< "handle_pg_trim " << *m << " from " << m->get_source()
						<< dendl;
	...
	if (m->pgid.preferred() >= 0) {
		dout(10)
					<< "ignoring localized pg " << m->pgid << dendl;
		return;
	}
	...
	if (!_have_pg(m->pgid)) {
		dout(10)
					<< " don't have pg " << m->pgid << dendl;
	} else {
		PG *pg = _lookup_lock_pg(m->pgid);
		if (m->epoch < pg->info.history.same_interval_since) {
			dout(10)
						<< *pg << " got old trim to " << m->trim_to
								<< ", ignoring" << dendl;
			pg->unlock();
			return;
		}
		...
		if (pg->is_primary()) {
			...
			if (pg->calc_min_last_complete_ondisk()) {
				dout(10)
							<< *pg << " min lcod now "
									<< pg->min_last_complete_ondisk << dendl;
				pg->trim_peers();
			}
		} else {
...
void OSD::handle_pg_backfill_reserve(OpRequestRef op) {
	...
	if (m->type == MBackfillReserve::REQUEST) {
		evt = PG::CephPeeringEvtRef(
				new PG::CephPeeringEvt(m->query_epoch, m->query_epoch,
...
void OSD::handle_pg_query(OpRequestRef op) {
	...
	dout(7)
				<< "handle_pg_query from " << m->get_source() << " epoch "
						<< m->get_epoch() << dendl;
	int from = m->get_source().num();
	...
	for (map<spg_t, pg_query_t>::iterator it = m->pg_list.begin();
		...
		if (pgid.preferred() >= 0) {
			dout(10)
						<< "ignoring localized pg " << pgid << dendl;
			continue;
		}
		...
		if (service.splitting(pgid)) {
			peering_wait_for_split[pgid].push_back(
					PG::CephPeeringEvtRef(
							new PG::CephPeeringEvt(it->second.epoch_sent,
									it->second.epoch_sent,
									PG::MQuery(
											pg_shard_t(from, it->second.from),
											it->second,
											it->second.epoch_sent))));
			continue;
		}
		...
		bool valid_history = project_pg_history(pgid, history,
				it->second.epoch_sent, up, up_primary, acting, acting_primary);

		if (!valid_history
				|| it->second.epoch_sent < history.same_interval_since) {
			dout(10)
						<< " pg " << pgid << " dne, and pg has changed in "
								<< history.same_interval_since << " (msg from "
								<< it->second.epoch_sent << ")" << dendl;
			continue;
		}
		...
		if (it->second.type == pg_query_t::LOG
			...
			ConnectionRef con = service.get_con_osd_cluster(from,
					osdmap->get_epoch());
			if (con) {
				MOSDPGLog *mlog = new MOSDPGLog(it->second.from, it->second.to,
						osdmap->get_epoch(), empty, it->second.epoch_sent);
			...
			notify_list[from].push_back(
					make_pair(
							pg_notify_t(it->second.from, it->second.to,
									it->second.epoch_sent, osdmap->get_epoch(),
									empty), pg_interval_map_t()));
		}
	}
...
void OSD::handle_pg_remove(OpRequestRef op) {
	...
	dout(7)
				<< "handle_pg_remove from " << m->get_source() << " on "
	...
	for (vector<spg_t>::iterator it = m->pg_list.begin();
		...
		if (pg_map.count(pgid) == 0) {
			dout(10)
						<< " don't have pg " << pgid << dendl;
			continue;
		}
		dout(5)
					<< "queue_pg_for_deletion: " << pgid << dendl;
		...
		osdmap->pg_to_up_acting_osds(pgid.pgid, &up, &up_primary, &acting,
				&acting_primary);
		bool valid_history = project_pg_history(pg->info.pgid, history,
				pg->get_osdmap()->get_epoch(), up, up_primary, acting,
				acting_primary);
		if (valid_history && history.same_interval_since <= m->get_epoch()) {
...
void OSD::_remove_pg(PG *pg) {
	...
	store->queue_transaction(pg->osr.get(), rmt,
			new ObjectStore::C_DeleteTransactionHolder<SequencerRef>(rmt,
					pg->osr), new ContainerContext<SequencerRef>(pg->osr));

	DeletingStateRef deleting = service.deleting_pgs.lookup_or_create(
			pg->info.pgid, make_pair(pg->info.pgid, PGRef(pg)));
...
void OSD::check_replay_queue() {
	...
	while (!replay_queue.empty() && replay_queue.front().second <= now) {
		pgids.push_back(replay_queue.front());
		replay_queue.pop_front();
	}
	replay_queue_lock.Unlock();

	for (list<pair<spg_t, utime_t> >::iterator p = pgids.begin();
			p != pgids.end(); ++p) {
		spg_t pgid = p->first;
		...
		if (pg_map.count(pgid)) {
			...
			pg_map_lock.unlock();
			dout(10)
						<< "check_replay_queue " << *pg << dendl;
			if (pg->is_active() && pg->is_replay() && pg->is_primary()
			...
			pg_map_lock.unlock();
			dout(10)
						<< "check_replay_queue pgid " << pgid << " (not found)"
								<< dendl;
		}
	}
}
...
bool OSD::_recover_now() {
	if (recovery_ops_active >= cct->_conf->osd_recovery_max_active) {
		dout(15)
					<< "_recover_now active " << recovery_ops_active
							<< " >= max " << cct->_conf->osd_recovery_max_active
							<< dendl;
		return false;
	}
	if (ceph_clock_now(cct) < defer_recovery_until) {
		dout(15)
					<< "_recover_now defer until " << defer_recovery_until
							<< dendl;
		return false;
	}
...
void OSD::do_recovery(PG *pg, ThreadPool::TPHandle &handle) {
	...
	if (max > 0) {
		...
		dout(10)
					<< "do_recovery can start 0 (" << recovery_ops_active << "/"
							<< cct->_conf->osd_recovery_max_active << " rops)"
							<< dendl;
	}
	recovery_wq.unlock();
	...
	if (max <= 0) {
		dout(10)
							<< "do_recovery raced and failed to start anything; requeuing "
							<< *pg << dendl;
		...
		dout(10)
					<< "do_recovery starting " << max << " " << *pg << dendl;
		...
		dout(10)
					<< "do_recovery started " << started << "/" << max << " on "
							<< *pg << dendl;
		...
		if (!more && pg->have_unfound()) {
			...
			if (rctx.query_map->empty()) {
				dout(10)
									<< "do_recovery  no luck, giving up on this pg for now"
									<< dendl;
				recovery_wq.lock();
...
void OSD::finish_recovery_op(PG *pg, const hobject_t& soid, bool dequeue) {
	recovery_wq.lock();
	dout(10)
				<< "finish_recovery_op " << *pg << " " << soid << " dequeue="
						<< dequeue << " (" << recovery_ops_active << "/"
						<< cct->_conf->osd_recovery_max_active << " rops)"
						<< dendl;
...
public:
	...
	void finish(ThreadPool::TPHandle& tp) {
		OSD::Session *session = static_cast<OSD::Session *>(con->get_priv());
		if (session) {
			session->sent_epoch_lock.Lock();
		}
		osd->service.share_map(name, con.get(), map_epoch, osdmap,
...
struct send_map_on_destruct {
	...
	ConnectionRef con;
	OSDMapRef osdmap;
	epoch_t map_epoch;bool should_send;
	send_map_on_destruct(OSD *osd, Message *m, OSDMapRef& osdmap,
			epoch_t map_epoch) :
			osd(osd), name(m->get_source()), con(m->get_connection()), osdmap(
					osdmap), map_epoch(map_epoch), should_send(true) {
	}
	~send_map_on_destruct() {
		if (!should_send)
			return;
		osd->service.op_gen_wq.queue(
				new C_SendMap(osd, name, con, osdmap, map_epoch));
	}
};
...
void OSD::handle_op(OpRequestRef& op, OSDMapRef& osdmap) {
	...
	if (m->get_oid().name.size() > max_name_len) {
		dout(4)
					<< "handle_op '" << m->get_oid().name << "' is longer than "
	...
	if (osdmap->is_blacklisted(m->get_source_addr())) {
		dout(4)
					<< "handle_op " << m->get_source_addr() << " is blacklisted"
							<< dendl;
	...
	share_map.should_send = service.should_share_map(m->get_source(),
			m->get_connection().get(), m->get_map_epoch(), osdmap,
			&client_session->last_sent_epoch);
	if (client_session) {
	...
	if (cct->_conf->osd_debug_drop_op_probability > 0
		...
		if ((double) rand() / (double) RAND_MAX
				< cct->_conf->osd_debug_drop_op_probability) {
			dout(0)
						<< "handle_op DEBUG artificially dropping op " << *m
								<< dendl;
			return;
		}
	}
	...
	if (op->may_write()) {
		...
		if (cct->_conf->osd_max_write_size
			...
			derr
						<< "handle_op msg data len " << m->get_data_len()
								<< " > osd_max_write_size "
	...
	if (!send_map) {
		dout(7)
							<< "don't have sender's osdmap; assuming it was valid and that"
							<< " client will resend" << dendl;
		return;
	}
	if (!send_map->have_pg_pool(pgid.pool())) {
		dout(7)
					<< "dropping request; pool did not exist" << dendl;
		clog->warn() << m->get_source_inst() << " invalid " << m->get_reqid()
				<< " pg " << m->get_pg() << " to osd." << whoami << " in e"
				<< osdmap->get_epoch() << ", client e" << m->get_map_epoch()
	...
	if (!send_map->osd_is_valid_op_target(pgid.pgid, whoami)) {
		dout(7)
					<< "we are invalid target" << dendl;
		clog->warn() << m->get_source_inst() << " misdirected "
				<< m->get_reqid() << " pg " << m->get_pg() << " to osd."
				<< whoami << " in e" << osdmap->get_epoch() << ", client e"
				<< m->get_map_epoch() << " pg " << pgid << " features "
	...
	if (!osdmap->have_pg_pool(pgid.pool())
		...
		dout(7)
							<< "dropping; no longer have PG (or pool); client will retarget"
							<< dendl;
		return;
	}
...
void OSD::handle_replica_op(OpRequestRef& op, OSDMapRef& osdmap) {
	...
	dout(10)
				<< __func__ << " " << *m << " epoch " << m->map_epoch << dendl;
	epoch_t up_epoch = service.get_up_epoch();
	if (m->map_epoch < up_epoch) {
		dout(3)
					<< "replica op from before up" << dendl;
		return;
	}
	...
	should_share_map = service.should_share_map(m->get_source(),
			m->get_connection().get(), m->map_epoch, osdmap,
	...
	if (pg) {
		...
		C_SendMap *send_map = new C_SendMap(this, m->get_source(),
				m->get_connection(), osdmap, m->map_epoch);
...
void OSD::enqueue_op(PG *pg, OpRequestRef& op) {
	...
	dout(15)
				<< "enqueue_op " << op << " prio "
						<< op->get_req()->get_priority() << " cost "
						<< op->get_req()->get_cost() << " latency " << latency
						<< " " << *(op->get_req()) << dendl;
	pg->queue_op(op);
}
...
void OSD::ShardedOpWQ::_process(uint32_t thread_index, heartbeat_handle_d *hb) {
	...
	lgeneric_subdout(osd->cct, osd, 30)
				<< "dequeue status: ";
...
void OSD::ShardedOpWQ::_enqueue(pair<PGRef, OpRequestRef> item) {
	...
	if (priority >= CEPH_MSG_PRIO_LOW)
		sdata->pqueue.enqueue_strict(item.second->get_req()->get_source_inst(),
				priority, item);
	else
...
void OSD::dequeue_op(PGRef pg, OpRequestRef op, ThreadPool::TPHandle &handle) {
	...
	if (op->send_map_update) {
		...
		service.share_map(m->get_source(), m->get_connection().get(),
				op->sent_epoch, osdmap,
				session ? &session->last_sent_epoch : NULL);
		if (session) {
			session->sent_epoch_lock.Unlock();
			session->put();
...
const char** OSD::get_tracked_conf_keys() const {
	static const char* KEYS[] = { "osd_max_backfills",
			"osd_min_recovery_priority", "osd_op_complaint_time",
			"osd_op_log_threshold", "osd_op_history_size",
			"osd_op_history_duration", "osd_map_cache_size",
			"osd_map_max_advance", "osd_pg_epoch_persisted_max_stale",
			"osd_disk_thread_ioprio_class", "osd_disk_thread_ioprio_priority",
			// clog & admin clog
...
void OSD::update_log_config() {
	...
	if (parse_log_client_options(g_ceph_context, log_to_monitors, log_to_syslog,
		...
		clog->update_config(log_to_monitors, log_to_syslog, log_channel,
				log_prio);
	derr
				<< "log_to_monitors " << log_to_monitors << dendl;
}
...
void OSD::check_config() {
	...
	if (g_conf->osd_map_cache_size
			<= (int) g_conf->osd_pg_epoch_persisted_max_stale + 2) {
		clog->warn() << "osd_map_cache_size (" << g_conf->osd_map_cache_size
...
void OSD::set_disk_tp_priority() {
	dout(10)
				<< __func__ << " class "
						<< cct->_conf->osd_disk_thread_ioprio_class
						<< " priority "
						<< cct->_conf->osd_disk_thread_ioprio_priority << dendl;
	int cls = ceph_ioprio_string_to_class(
			cct->_conf->osd_disk_thread_ioprio_class);
	if (cls < 0)
		derr
					<< __func__ << cpp_strerror(cls) << ": "
							<< "osd_disk_thread_ioprio_class is "
							<< cct->_conf->osd_disk_thread_ioprio_class
							<< " but only the following values are allowed: idle, be or rt"
							<< dendl;
	else
...
int OSD::init_op_flags(OpRequestRef& op) {
	...
	for (iter = m->ops.begin(); iter != m->ops.end(); ++iter) {
		...
		case CEPH_OSD_OP_CALL: {
			...
			dout(10)
						<< "class " << cname << " method " << mname << " flags="
								<< (is_read ? "r" : "") << (is_write ? "w" : "")
								<< dendl;
			if (is_read)
				op->set_class_read();
			if (is_write)
...
bool OSD::RecoveryWQ::_enqueue(PG *pg) {
	if (!pg->recovery_item.is_on_list()) {
		...
		if (osd->cct->_conf->osd_recovery_delay_start > 0) {
			...
			osd->defer_recovery_until +=
					osd->cct->_conf->osd_recovery_delay_start;
		}
		return true;
	}
	return false;
