		...
		if (!p->second.tiers.empty()) {
			...
			for (set<uint64_t>::const_iterator q = base.tiers.begin();
				...
				if (tier->tier_of != p->first) {
					lderr(cct)
								<< __func__ << " " << r->first << " tier_of != "
										<< p->first << dendl;
					return -EIO;
				}

				ldout(cct, 10)
							<< __func__ << " from " << p->first << " to "
									<< r->first << dendl;
...
bool OSDMap::containing_subtree_is_down(CephContext *cct, int id,
		int subtree_type, set<int> *down_cache) const {
	...
	while (true) {
		...
		if (!subtree_is_down(current, down_cache)) {
			ldout(cct, 30)
						<< "containing_subtree_is_down(" << id << ") = false"
								<< dendl;
			return false;
		}
...
void OSDMap::Incremental::encode_client_old(bufferlist& bl) const {
	...
	n = old_pools.size();
	::encode(n, bl);
	for (set<int64_t>::iterator p = old_pools.begin(); p != old_pools.end();
			++p) {
		n = *p;
		::encode(n, bl);
	}
...
void OSDMap::Incremental::encode_classic(bufferlist& bl,
...
void OSDMap::Incremental::dump(Formatter *f) const {
	...
	for (map<int32_t, entity_addr_t>::const_iterator p = new_up_client.begin();
			p != new_up_client.end(); ++p) {
		...
		f->dump_stream("heartbeat_back_addr")
				<< new_hb_back_up.find(p->first)->second;
	...
	for (map<int32_t, uint32_t>::const_iterator p = new_up_thru.begin();
			p != new_up_thru.end(); ++p) {
		f->open_object_section("osd");
		f->dump_int("osd", p->first);
	...
	for (map<int32_t, uint32_t>::const_iterator p = new_lost.begin();
			p != new_lost.end(); ++p) {
		f->open_object_section("osd");
		f->dump_int("osd", p->first);
	...
	for (map<int32_t, pair<epoch_t, epoch_t> >::const_iterator p =
			new_last_clean_interval.begin(); p != new_last_clean_interval.end();
			++p) {
		f->open_object_section("osd");
		f->dump_int("osd", p->first);
	...
	for (vector<entity_addr_t>::const_iterator p = old_blacklist.begin();
			p != old_blacklist.end(); ++p)
		f->dump_stream("addr") << *p;
	f->close_section();

	f->open_array_section("new_xinfo");
	for (map<int32_t, osd_xinfo_t>::const_iterator p = new_xinfo.begin();
			p != new_xinfo.end(); ++p) {
	...
	for (vector<string>::const_iterator p = old_erasure_code_profiles.begin();
			p != old_erasure_code_profiles.end(); ++p) {
...
void OSDMap::set_epoch(epoch_t e) {
	epoch = e;
	for (map<int64_t, pg_pool_t>::iterator p = pools.begin(); p != pools.end();
			++p)
...
void OSDMap::get_blacklist(list<pair<entity_addr_t, utime_t> > *bl) const {
	for (ceph::unordered_map<entity_addr_t, utime_t>::const_iterator it =
			blacklist.begin(); it != blacklist.end(); ++it) {
		bl->push_back(*it);
	}
}
...
unsigned OSDMap::get_num_up_osds() const {
	unsigned n = 0;
	for (int i = 0; i < max_osd; i++)
		if ((osd_state[i] & CEPH_OSD_EXISTS) && (osd_state[i] & CEPH_OSD_UP))
			n++;
	return n;
}
...
unsigned OSDMap::get_num_in_osds() const {
	unsigned n = 0;
	for (int i = 0; i < max_osd; i++)
		if ((osd_state[i] & CEPH_OSD_EXISTS) && get_weight(i) != CEPH_OSD_OUT)
			n++;
	return n;
}
...
uint64_t OSDMap::get_features(int entity_type, uint64_t *pmask) const {
	...
	for (map<int64_t, pg_pool_t>::const_iterator p = pools.begin();
		...
		if (p->second.is_erasure() && entity_type != CEPH_ENTITY_TYPE_CLIENT) { // not for clients
			features |= CEPH_FEATURE_OSD_ERASURE_CODES;
		}
		if (!p->second.tiers.empty() || p->second.is_tier()) {
			features |= CEPH_FEATURE_OSD_CACHEPOOL;
		}
		int ruleid = crush->find_rule(p->second.get_crush_ruleset(),
				p->second.get_type(), p->second.get_size());
		if (ruleid >= 0) {
	...
	if (entity_type == CEPH_ENTITY_TYPE_OSD) {
		for (map<string, map<string, string> >::const_iterator p =
				erasure_code_profiles.begin(); p != erasure_code_profiles.end();
				++p) {
			const map<string, string> &profile = p->second;
			map<string, string>::const_iterator plugin = profile.find("plugin");
			if (plugin != profile.end()
					&& (plugin->second == "isa" || plugin->second == "lrc"))
...
void OSDMap::remove_redundant_temporaries(CephContext *cct,
		const OSDMap& osdmap, OSDMap::Incremental *pending_inc) {
	ldout(cct, 10)
				<< "remove_redundant_temporaries" << dendl;

	for (map<pg_t, vector<int32_t> >::iterator p = osdmap.pg_temp->begin();
			p != osdmap.pg_temp->end(); ++p) {
		if (pending_inc->new_pg_temp.count(p->first) == 0) {
			...
			if (raw_up == p->second) {
				ldout(cct, 10)
							<< " removing unnecessary pg_temp " << p->first
									<< " -> " << p->second << dendl;
	...
	if (!osdmap.primary_temp->empty()) {
		...
		for (map<pg_t, int32_t>::iterator p = osdmap.primary_temp->begin();
			...
			if (pending_inc->new_primary_temp.count(p->first) == 0) {
				...
				templess.pg_to_acting_osds(p->first, &templess_up,
						&templess_primary);
				if (real_primary == templess_primary) {
					ldout(cct, 10)
								<< " removing unnecessary primary_temp "
										<< p->first << " -> " << p->second
										<< dendl;
...
void OSDMap::remove_down_temps(CephContext *cct, const OSDMap& osdmap,
		Incremental *pending_inc) {
	ldout(cct, 10)
				<< "remove_down_pg_temp" << dendl;
	OSDMap tmpmap;
	tmpmap.deepish_copy_from(osdmap);
	tmpmap.apply_incremental(*pending_inc);

	for (map<pg_t, vector<int32_t> >::iterator p = tmpmap.pg_temp->begin();
			p != tmpmap.pg_temp->end(); ++p) {
		unsigned num_up = 0;
...
int OSDMap::apply_incremental(const Incremental &inc) {
	...
	for (map<int32_t, uint32_t>::const_iterator i =
			inc.new_primary_affinity.begin();
			i != inc.new_primary_affinity.end(); ++i) {
	...
	for (map<string, map<string, string> >::const_iterator i =
			...
			i != inc.new_erasure_code_profiles.end(); ++i) {
		set_erasure_code_profile(i->first, i->second);
	}

	for (vector<string>::const_iterator i =
			inc.old_erasure_code_profiles.begin();
			i != inc.old_erasure_code_profiles.end(); ++i)
		erasure_code_profiles.erase(*i);

	// up/down
	for (map<int32_t, uint8_t>::const_iterator i = inc.new_state.begin();
			i != inc.new_state.end(); ++i) {
		int s = i->second ? i->second : CEPH_OSD_UP;
		if ((osd_state[i->first] & CEPH_OSD_UP) && (s & CEPH_OSD_UP)) {
			osd_info[i->first].down_at = epoch;
			osd_xinfo[i->first].down_stamp = modified;
		}
		if ((osd_state[i->first] & CEPH_OSD_EXISTS) && (s & CEPH_OSD_EXISTS))
			(*osd_uuid)[i->first] = uuid_d();
		osd_state[i->first] ^= s;
	}
	for (map<int32_t, entity_addr_t>::const_iterator i =
			inc.new_up_client.begin(); i != inc.new_up_client.end(); ++i) {
		...
		if (inc.new_hb_back_up.empty())
			osd_addrs->hb_back_addr[i->first].reset(
					new entity_addr_t(i->second)); //this is a backward-compatibility hack
		else
			osd_addrs->hb_back_addr[i->first].reset(
					new entity_addr_t(
							inc.new_hb_back_up.find(i->first)->second));
		map<int32_t, entity_addr_t>::const_iterator j =
				inc.new_hb_front_up.find(i->first);
		if (j != inc.new_hb_front_up.end())
			osd_addrs->hb_front_addr[i->first].reset(
					new entity_addr_t(j->second));
		else
			osd_addrs->hb_front_addr[i->first].reset();
	...
	for (map<int32_t, entity_addr_t>::const_iterator i =
			inc.new_up_cluster.begin(); i != inc.new_up_cluster.end(); ++i)
	...
	for (map<int32_t, epoch_t>::const_iterator i = inc.new_up_thru.begin();
			i != inc.new_up_thru.end(); ++i)
		osd_info[i->first].up_thru = i->second;
	for (map<int32_t, pair<epoch_t, epoch_t> >::const_iterator i =
			inc.new_last_clean_interval.begin();
			i != inc.new_last_clean_interval.end(); ++i) {
		osd_info[i->first].last_clean_begin = i->second.first;
		osd_info[i->first].last_clean_end = i->second.second;
	}
	for (map<int32_t, epoch_t>::const_iterator p = inc.new_lost.begin();
			p != inc.new_lost.end(); ++p)
		osd_info[p->first].lost_at = p->second;

	// xinfo
	for (map<int32_t, osd_xinfo_t>::const_iterator p = inc.new_xinfo.begin();
			p != inc.new_xinfo.end(); ++p)
		osd_xinfo[p->first] = p->second;

	// uuid
	for (map<int32_t, uuid_d>::const_iterator p = inc.new_uuid.begin();
			p != inc.new_uuid.end(); ++p)
		(*osd_uuid)[p->first] = p->second;

	// pg rebuild
	for (map<pg_t, vector<int> >::const_iterator p = inc.new_pg_temp.begin();
			p != inc.new_pg_temp.end(); ++p) {
		if (p->second.empty())
	...
	// blacklist
	for (map<entity_addr_t, utime_t>::const_iterator p =
			inc.new_blacklist.begin(); p != inc.new_blacklist.end(); ++p) {
		blacklist[p->first] = p->second;
		new_blacklist_entries = true;
	}
	for (vector<entity_addr_t>::const_iterator p = inc.old_blacklist.begin();
			p != inc.old_blacklist.end(); ++p)
		blacklist.erase(*p);
...
int OSDMap::_pg_to_osds(const pg_pool_t& pool, pg_t pg, vector<int> *osds,
...
void OSDMap::_apply_primary_affinity(ps_t seed, const pg_pool_t& pool,
	...
	int osd = sum % size;
	for (int i = 0; i < size - 1; i++, osd++){
		printf("%d,%d\n", i, osds->at(osd % size));
		if (osd != *primary) new_osds.push_back(osds->at(osd % size));
		else new_osds.push_back(osds->at((++osd) % size));
	}
	osds->swap(new_osds);
}
...
void OSDMap::dump_erasure_code_profiles(
		const map<string, map<string, string> > &profiles, Formatter *f) {
...
struct qi {
	...
	float weight;
	qi() :
			item(0), depth(0), weight(0) {
	}
	qi(int i, int d, float w) :
			item(i), depth(d), weight(w) {
	}
};
...
void OSDMap::print_pools(ostream& out) const {
	for (map<int64_t, pg_pool_t>::const_iterator p = pools.begin();
		...
		if (pni != pool_name.end())
			name = pni->second;
		out << "pool " << p->first << " '" << name << "' " << p->second << "\n";
		for (map<snapid_t, pool_snap_info_t>::const_iterator q =
				p->second.snaps.begin(); q != p->second.snaps.end(); ++q)
	...
	for (int i = 0; i < get_max_osd(); i++) {
		if (exists(i)) {
			...
			out << " " << get_addr(i) << " " << get_cluster_addr(i) << " "
					<< get_hb_back_addr(i) << " " << get_hb_front_addr(i);
			set<string> st;
			get_state(i, st);
			out << " " << st;
	...
	for (set<int>::iterator p = roots.begin(); p != roots.end(); ++p) {
		...
		while (!q.empty()) {
			...
			for (int k = s - 1; k >= 0; k--) {
				...
				q.push_front(
						qi(item, depth + 1,
								(float) crush->get_bucket_item_weight(cur, k)
										/ (float) 0x10000));
				if (f)
			...
			if (out)
				*out << crush->get_type_name(type) << " "
						<< crush->get_item_name(cur) << "\n";
			if (f) {
				f->close_section();
			}
...
void OSDMap::print_summary(Formatter *f, ostream& out) const {
	if (f) {
		...
		f->dump_bool("nearfull",
				test_flag(CEPH_OSDMAP_NEARFULL) ? true : false);
		f->close_section();
	} else {
		out << "     osdmap e" << get_epoch() << ": " << get_num_osds()
				<< " osds: " << get_num_up_osds() << " up, "
				<< get_num_in_osds() << " in\n";
		if (flags)
			out << "            flags " << get_flag_string() << "\n";
	}
...
void OSDMap::print_oneline_summary(ostream& out) const {
	out << "e" << get_epoch() << ": " << get_num_osds() << " osds: "
...
int OSDMap::build_simple(CephContext *cct, epoch_t e, uuid_d &fsid, int nosd,
		int pg_bits, int pgp_bits) {
	ldout(cct, 10)
				<< "build_simple on " << num_osd << " osds with " << pg_bits
						<< " pg bits per osd, " << dendl;
	epoch = e;
	set_fsid(fsid);
	...
	if (nosd >= 0) {
		...
		for (vector<string>::iterator i = sections.begin(); i != sections.end();
				++i) {
			if (i->find("osd.") != 0)
				continue;
	...
	int const default_replicated_ruleset =
			crush->get_osd_pool_default_crush_replicated_ruleset(cct);
	assert(default_replicated_ruleset >= 0);

	for (vector<string>::iterator p = pool_names.begin(); p != pool_names.end();
			++p) {
		...
		pools[pool].hint_size = cct->_conf->osd_pool_default_hint_size;
...
int OSDMap::get_erasure_code_profile_default(CephContext *cct,
		map<string, string> &profile_map, ostream *ss) {
...
int OSDMap::build_simple_crush_rulesets(CephContext *cct, CrushWrapper& crush,
		const string& root, ostream *ss) {
	string failure_domain = crush.get_type_name(
