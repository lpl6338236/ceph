...
ObjectCacher::BufferHead *ObjectCacher::Object::split(BufferHead *left,
	...
	if (!left->waitfor_read.empty()) {
		...
		for (map<loff_t, list<Context*> >::iterator p = start_remove;
			...
			ldout(oc->cct, 20)
						<< "split  moving waiters at byte " << p->first
								<< " to right bh" << dendl;
	...
	ldout(oc->cct, 20)
				<< "split    left is " << *left << dendl;
	ldout(oc->cct, 20)
				<< "split   right is " << *right << dendl;
	return right;
}
...
void ObjectCacher::Object::try_merge_bh(BufferHead *bh) {
	...
	assert(p->second == bh);
	++p;
	if (p != data.end() && p->second->start() == bh->end()
			&& p->second->get_state() == bh->get_state())
 ...
 */
int ObjectCacher::Object::map_read(OSDRead *rd, map<loff_t, BufferHead*>& hits,
		map<loff_t, BufferHead*>& missing, map<loff_t, BufferHead*>& rx,
	...
	for (vector<ObjectExtent>::iterator ex_it = rd->extents.begin();
		...
		ldout(oc->cct, 10)
					<< "map_read " << ex_it->oid << " " << ex_it->offset << "~"
							<< ex_it->length << dendl;
		...
		while (left > 0) {
			...
			if (p == data.end()) {
				...
				if (complete) {
					...
					hits[cur] = n;
					ldout(oc->cct, 20)
								<< "map_read miss+complete+zero " << left
										<< " left, " << *n << dendl;
				} else {
					missing[cur] = n;
					ldout(oc->cct, 20)
								<< "map_read miss " << left << " left, " << *n
										<< dendl;
				}
				cur += left;
			...
			if (p->first <= cur) {
				...
				if (e->is_clean() || e->is_dirty() || e->is_tx()
						|| e->is_zero()) {
					hits[cur] = e;     // readable!
					ldout(oc->cct, 20)
								<< "map_read hit " << *e << dendl;
				} else if (e->is_rx()) {
				...
				if (complete) {
					...
					hits[cur] = n;
					ldout(oc->cct, 20)
								<< "map_read gap+complete+zero " << *n << dendl;
				} else {
					missing[cur] = n;
					ldout(oc->cct, 20)
								<< "map_read gap " << *n << dendl;
				}
...
void ObjectCacher::Object::audit_buffers() {
	...
	for (map<loff_t, BufferHead*>::const_iterator it = data.begin();
		...
		if (it->first != it->second->start()) {
			lderr(oc->cct)
						<< "AUDIT FAILURE: map position " << it->first
								<< " does not match bh start position: "
								<< *it->second << dendl;assert(
					it->first == it->second->start());
		}
		if (it->first < offset) {
			lderr(oc->cct)
						<< "AUDIT FAILURE: " << it->first << " " << *it->second
								<< " overlaps with previous bh "
								<< *((--it)->second) << dendl;assert(
					it->first >= offset);
		}
		...
		for (w_it = bh->waitfor_read.begin(); w_it != bh->waitfor_read.end();
			...
			if (w_it->first < bh->start()
				...
				lderr(oc->cct)
							<< "AUDIT FAILURE: waiter at " << w_it->first
...
ObjectCacher::BufferHead *ObjectCacher::Object::map_write(OSDWrite *wr) {
	...
	for (vector<ObjectExtent>::iterator ex_it = wr->extents.begin();
		...
		ldout(oc->cct, 10)
					<< "map_write oex " << ex_it->oid << " " << ex_it->offset
							<< "~" << ex_it->length << dendl;
		...
		while (left > 0) {
			...
			if (p == data.end()) {
				if (final == NULL) {
					...
					ldout(oc->cct, 10)
								<< "map_write adding trailing bh " << *final
										<< dendl;
				} else {
			...
			ldout(oc->cct, 10)
						<< "cur is " << cur << ", p is " << *p->second << dendl;
						//oc->verify_stats();

			if (p->first <= cur) {
				BufferHead *bh = p->second;
				ldout(oc->cct, 10)
							<< "map_write bh " << *bh << " intersected" << dendl;
	...
	assert(final);
	ldout(oc->cct, 10)
				<< "map_write final is " << *final << dendl;
...
void ObjectCacher::Object::discard(loff_t off, loff_t len) {
	assert(oc->lock.is_locked());
	ldout(oc->cct, 10)
				<< "discard " << *this << " " << off << "~" << len << dendl;

	if (!exists) {
		ldout(oc->cct, 10)
					<< " setting exists on " << *this << dendl;
		exists = true;
	}
	if (complete) {
		ldout(oc->cct, 10)
					<< " clearing complete on " << *this << dendl;
		complete = false;
	}
...
ObjectCacher::ObjectCacher(CephContext *cct_, string name, WritebackHandler& wb,
		Mutex& l, flush_set_callback_t flush_callback, void *flush_callback_arg,
		uint64_t max_bytes, uint64_t max_objects, uint64_t max_dirty,
		uint64_t target_dirty, double max_dirty_age, bool block_writes_upfront) :
		perfcounter(NULL), cct(cct_), writeback_handler(wb), name(name), lock(
				l), max_dirty(max_dirty), target_dirty(target_dirty), max_size(
				max_bytes), max_objects(max_objects), block_writes_upfront(
				block_writes_upfront), flush_set_callback(flush_callback), flush_set_callback_arg(
				flush_callback_arg), last_read_tid(0), flusher_stop(false), flusher_thread(
				this), finisher(cct), stat_clean(0), stat_zero(0), stat_dirty(
				0), stat_rx(0), stat_tx(0), stat_missing(0), stat_error(0), stat_dirty_waiting(
				0), reads_outstanding(0) {
...
ObjectCacher::Object *ObjectCacher::get_object(sobject_t oid, ObjectSet *oset,
		object_locator_t &l, uint64_t truncate_size, uint64_t truncate_seq) {
...
void ObjectCacher::bh_read(BufferHead *bh, char* hint) {
	assert(lock.is_locked());
	ldout(cct, 7)
				<< "bh_read on " << *bh << " outstanding reads "
	...
	C_ReadFinish *onfinish = new C_ReadFinish(this, bh->ob, bh->last_read_tid,
			bh->start(), bh->length());
	// go
	writeback_handler.read(bh->ob->get_oid(), bh->ob->get_oloc(), bh->start(),
			bh->length(), bh->ob->get_snap(), &onfinish->bl,
			bh->ob->truncate_size, bh->ob->truncate_seq, onfinish, hint);

	++reads_outstanding;
}

void ObjectCacher::bh_read_finish(int64_t poolid, sobject_t oid, ceph_tid_t tid,
		loff_t start, uint64_t length, bufferlist &bl, int r,
		bool trust_enoent) {
	assert(lock.is_locked());
	ldout(cct, 7)
				<< "bh_read_finish " << oid << " tid " << tid << " " << start
	...
	if (bl.length() < length) {
		...
		bp.zero();
		ldout(cct, 7)
					<< "bh_read_finish " << oid << " padding " << start << "~"
							<< length << " with " << bp.length()
	...
	if (objects[poolid].count(oid) == 0) {
		ldout(cct, 7)
					<< "bh_read_finish no object cache" << dendl;
	} else {
		...
		if (r == -ENOENT && !ob->complete) {
			...
			for (map<loff_t, BufferHead*>::iterator p = ob->data.begin();
				...
				for (map<loff_t, list<Context*> >::iterator p =
						bh->waitfor_read.begin(); p != bh->waitfor_read.end();
						++p)
			...
			if (trust_enoent) {
				ldout(cct, 7)
									<< "bh_read_finish ENOENT, marking complete and !exists on "
									<< *ob << dendl;
				ob->complete = true;
				ob->exists = false;
		...
		while (true) {
			...
			if (opos >= start + (loff_t) length) {
				ldout(cct, 20)
							<< "break due to opos " << opos
									<< " >= start+length " << start << "+"
									<< length << "=" << start + (loff_t) length
									<< dendl;
				break;
			}
			...
			BufferHead *bh = p->second;
			ldout(cct, 20)
						<< "checking bh " << *bh << dendl;

						// finishers?
			for (map<loff_t, list<Context*> >::iterator it =
					bh->waitfor_read.begin(); it != bh->waitfor_read.end();
					++it)
			...
			if (bh->start() > opos) {
				ldout(cct, 1)
							<< "bh_read_finish skipping gap " << opos << "~"
									<< bh->start() - opos << dendl;
				opos = bh->start();
				continue;
			}
			...
			if (!bh->is_rx()) {
				ldout(cct, 10)
							<< "bh_read_finish skipping non-rx " << *bh << dendl;
				opos = bh->end();
				continue;
			}
			...
			if (bh->last_read_tid != tid) {
				ldout(cct, 10)
							<< "bh_read_finish bh->last_read_tid "
			...
			if (r == -ENOENT) {
				if (trust_enoent) {
					...
					ldout(cct, 10)
										<< "skipping unstrusted -ENOENT and will retry for "
										<< *bh << dendl;
				}
				continue;
			}
...
void ObjectCacher::bh_write(BufferHead *bh, char* hint) {
	...
	ceph_tid_t tid = writeback_handler.write(bh->ob->get_oid(),
			bh->ob->get_oloc(), bh->start(), bh->length(), bh->snapc, bh->bl,
			bh->last_write, bh->ob->truncate_size, bh->ob->truncate_seq,
			oncommit, hint);
	ldout(cct, 20)
				<< " tid " << tid << " on " << bh->ob->get_oid() << dendl;
...
void ObjectCacher::bh_write_commit(int64_t poolid, sobject_t oid, loff_t start,
	...
	assert(lock.is_locked());
	ldout(cct, 7)
				<< "bh_write_commit " << oid << " tid " << tid << " " << start
						<< "~" << length << " returned " << r << dendl;

	if (objects[poolid].count(oid) == 0) {
		ldout(cct, 7)
					<< "bh_write_commit no object cache" << dendl;
	} else {
		Object *ob = objects[poolid][oid];
		...
		if (!ob->exists) {
			ldout(cct, 10)
						<< "bh_write_commit marking exists on " << *ob << dendl;
			ob->exists = true;

			if (writeback_handler.may_copy_on_write(ob->get_oid(), start,
					length, ob->get_snap())) {
				ldout(cct, 10)
									<< "bh_write_commit may copy on write, clearing complete on "
									<< *ob << dendl;
				ob->complete = false;
			}
		}
		...
		for (map<loff_t, BufferHead*>::iterator p = ob->data_lower_bound(start);
			...
			if (bh->start() < start && bh->end() > start + (loff_t) length) {
				ldout(cct, 20)
							<< "bh_write_commit skipping " << *bh << dendl;
				continue;
			}
			...
			if (!bh->is_tx()) {
				ldout(cct, 10)
							<< "bh_write_commit skipping non-tx " << *bh
									<< dendl;
				continue;
			}
			...
			if (bh->last_write_tid != tid) {
				...
				ldout(cct, 10)
							<< "bh_write_commit newer tid on " << *bh << dendl;
				continue;
			}
			...
			if (r >= 0) {
				...
				mark_clean(bh);
				ldout(cct, 10)
							<< "bh_write_commit clean " << *bh << dendl;
			} else {
				mark_dirty(bh);
				ldout(cct, 10)
									<< "bh_write_commit marking dirty again due to error "
									<< *bh << " r = " << r << " "
...
void ObjectCacher::trim() {
	...
	while (get_stat_clean() > 0 && (uint64_t) get_stat_clean() > max_size) {
		...
		if (ob->complete) {
			ldout(cct, 10)
						<< "trim clearing complete on " << *ob << dendl;
			ob->complete = false;
		}
	}
...
bool external_call) {
	...
	for (vector<ObjectExtent>::iterator ex_it = rd->extents.begin();
		...
		if (oset->return_enoent && !o->exists) {
			...
			ldout(cct, 10)
						<< "readx  object !exists, 1 extent..." << dendl;
			...
			if (writeback_handler.may_copy_on_write(soid.oid, ex_it->offset,
				...
				if (wait) {
					ldout(cct, 10)
								<< "readx  waiting on tid " << o->last_write_tid
										<< " on " << *o << dendl;
			...
			for (map<loff_t, BufferHead*>::iterator bh_it = o->data.begin();
					bh_it != o->data.end(); ++bh_it) {
				ldout(cct, 20)
							<< "readx  ob has bh " << *bh_it->second << dendl;
			...
			if (allzero) {
				ldout(cct, 10)
							<< "readx  ob has all zero|rx, returning ENOENT"
									<< dendl;
				delete rd;
				return -ENOENT;
			}
		...
		if (!missing.empty() || !rx.empty()) {
			...
			for (map<loff_t, BufferHead*>::iterator bh_it = missing.begin();
				...
				uint64_t rx_bytes = static_cast<uint64_t>(stat_rx
						+ bh_it->second->length());
				if (!waitfor_read.empty() || rx_bytes > max_size) {
					...
					if (success) {
						ldout(cct, 10)
											<< "readx missed, waiting on cache to complete "
											<< waitfor_read.size()
											<< " blocked reads, "
											<< (MAX(rx_bytes, max_size)
													- max_size) << " read bytes"
											<< dendl;
					...
					bh_read(bh_it->second, rd->hint);
					if (success && onfinish) {
						ldout(cct, 10)
									<< "readx missed, waiting on "
											<< *bh_it->second << " off "
											<< bh_it->first << dendl;
			...
			for (map<loff_t, BufferHead*>::iterator bh_it = rx.begin();
				...
				if (success && onfinish) {
					ldout(cct, 10)
								<< "readx missed, waiting on " << *bh_it->second
			...
			for (map<loff_t, BufferHead*>::iterator bh_it = hits.begin();
				...
				ldout(cct, 10)
							<< "readx hit bh " << *bh_it->second << dendl;
	...
	if (rd->bl && !error) {
		...
		for (map<uint64_t, bufferlist>::iterator i = stripe_map.begin();
			...
			assert(pos == i->first);
			ldout(cct, 10)
						<< "readx  adding buffer len " << i->second.length()
								<< " at " << pos << dendl;
			pos += i->second.length();
		...
		ldout(cct, 10)
					<< "readx  result is " << rd->bl->length() << dendl;
	} else {
		ldout(cct, 10)
					<< "readx  no bufferlist ptr (readahead?), done." << dendl;
...
int ObjectCacher::_wait_for_write(OSDWrite *wr, uint64_t len, ObjectSet *oset,
		Mutex& lock, Context *onfreespace) {
	assert(lock.is_locked());
	int ret = 0;
	...
	if (max_dirty > 0) {
		...
		ldout(cct, 10)
					<< "wait_for_write waiting on write-thru of " << len
							<< " bytes" << dendl;
		if (block_writes_upfront) {
			while (!done)
				cond.Wait(lock);
			ldout(cct, 10)
						<< "wait_for_write woke up, ret " << ret << dendl;
			if (onfreespace)
	...
	if (get_stat_dirty() > 0 && (uint64_t) get_stat_dirty() > target_dirty) {
		ldout(cct, 10)
					<< "wait_for_write " << get_stat_dirty() << " > target "
...
void ObjectCacher::flusher_entry() {
	...
	while (!flusher_stop) {
		loff_t all = get_stat_tx() + get_stat_rx() + get_stat_clean()
				+ get_stat_dirty();
		ldout(cct, 11)
					<< "flusher " << all << " / " << max_size << ":  "
							<< get_stat_tx() << " tx, " << get_stat_rx()
							<< " rx, " << get_stat_clean() << " clean, "
							<< get_stat_dirty() << " dirty (" << target_dirty
		...
		if (actual > 0 && (uint64_t) actual > target_dirty) {
			...
			ldout(cct, 10)
						<< "flusher " << get_stat_dirty() << " dirty + "
								<< get_stat_dirty_waiting()
								<< " dirty_waiting > target " << target_dirty
			...
			while ((bh =
					static_cast<BufferHead*>(bh_lru_dirty.lru_get_next_expire()))
					!= 0 && bh->last_write < cutoff && --max > 0) {
				ldout(cct, 10)
							<< "flusher flushing aged dirty bh " << *bh << dendl;
				bh_write(bh);
			}
			if (!max) {
	...
	while (reads_outstanding > 0) {
		ldout(cct, 10)
					<< "Waiting for all reads to complete. Number left: "
							<< reads_outstanding << dendl;
		read_cond.Wait(lock);
	}
...
bool ObjectCacher::_flush_set_finish(C_GatherBuilder *gather,
	...
	ldout(cct, 10)
				<< "flush_set has no dirty|tx bhs" << dendl;
	onfinish->complete(0);
	return true;
}
...
bool ObjectCacher::flush_set(ObjectSet *oset, Context *onfinish) {
	...
	if (oset->objects.empty()) {
		ldout(cct, 10)
					<< "flush_set on " << oset << " dne" << dendl;
		onfinish->complete(0);
		return true;
	}

	ldout(cct, 10)
				<< "flush_set " << oset << dendl;
...
void ObjectCacher::purge_set(ObjectSet *oset) {
	...
	if (oset->objects.empty()) {
		ldout(cct, 10)
					<< "purge_set on " << oset << " dne" << dendl;
		return;
	}

	ldout(cct, 10)
				<< "purge_set " << oset << dendl;
...
loff_t ObjectCacher::release(Object *ob) {
	...
	if (ob->complete) {
		ldout(cct, 10)
					<< "release clearing complete on " << *ob << dendl;
		ob->complete = false;
	}
	if (!ob->exists) {
		ldout(cct, 10)
					<< "release setting exists on " << *ob << dendl;
		ob->exists = true;
	}
...
loff_t ObjectCacher::release_set(ObjectSet *oset) {
	...
	if (oset->objects.empty()) {
		ldout(cct, 10)
					<< "release_set on " << oset << " dne" << dendl;
		return 0;
	}

	ldout(cct, 10)
				<< "release_set " << oset << dendl;
...
uint64_t ObjectCacher::release_all() {
	...
	if (unclean) {
		ldout(cct, 10)
					<< "release_all unclean " << unclean << " bytes left"
							<< dendl;
	}
...
void ObjectCacher::clear_nonexistence(ObjectSet *oset) {
	assert(lock.is_locked());
	ldout(cct, 10)
				<< "clear_nonexistence() " << oset << dendl;

	for (xlist<Object*>::iterator p = oset->objects.begin(); !p.end(); ++p) {
		Object *ob = *p;
		if (!ob->exists) {
			ldout(cct, 10)
						<< " setting exists and complete on " << *ob << dendl;
			ob->exists = true;
			ob->complete = false;
		}
		for (xlist<C_ReadFinish*>::iterator q = ob->reads.begin(); !q.end();
				++q) {
			C_ReadFinish *comp = *q;
...
void ObjectCacher::discard_set(ObjectSet *oset, vector<ObjectExtent>& exls) {
	...
	if (oset->objects.empty()) {
		ldout(cct, 10)
					<< "discard_set on " << oset << " dne" << dendl;
		return;
	}

	ldout(cct, 10)
				<< "discard_set " << oset << dendl;

	bool were_dirty = oset->dirty_or_tx > 0;

	for (vector<ObjectExtent>::iterator p = exls.begin(); p != exls.end();
			++p) {
		ldout(cct, 10)
					<< "discard_set " << oset << " ex " << *p << dendl;
		ObjectExtent &ex = *p;
		sobject_t soid(ex.oid, CEPH_NOSNAP);
		if (objects[oset->poolid].count(soid) == 0)
			continue;
	...
	if (flush_set_callback && were_dirty && oset->dirty_or_tx == 0)
		flush_set_callback(flush_set_callback_arg, oset);
}
...
void ObjectCacher::verify_stats() const {
	...
	loff_t clean = 0, zero = 0, dirty = 0, rx = 0, tx = 0, missing = 0, error =
			0;
...
void ObjectCacher::bh_set_state(BufferHead *bh, int s) {
	...
	} else if (s != BufferHead::STATE_DIRTY
	...
	if ((s == BufferHead::STATE_TX || s == BufferHead::STATE_DIRTY)
		...
		dirty_or_tx_bh.insert(bh);
			...
			|| state == BufferHead::STATE_DIRTY) && s != BufferHead::STATE_TX
